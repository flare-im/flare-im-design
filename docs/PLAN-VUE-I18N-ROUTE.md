...
非注释中文=0（除 messages.ts zh locale + 语言选择器自称名 `简体中文`）。vue-tsc 净、14/14 vitest 过（2 个 collection 失败=`@flare-im/sdk/contract` 子路径解析，stash 验证为既存环境问题，与本改无关）。
...
- [x] **收尾 ✅**：全域 han 复扫=1（仅 `简体中文` 自称名，符合预期）；vue-tsc 净、14/14 vitest 过（2 collection 失败=`@flare-im/sdk/contract` 子路径解析，stash 验证既存、与本改无关）。
