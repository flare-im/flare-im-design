...
**P1-3 Message mapping drops state** [APP, FIXED in Round 2; ACK row-identity regression fixed in Round 6] The mapper now passes the core's `isEdited`, `replyTo`, `quotePreview`, `textPreview`, `attributes` (pinned), `localState` (upload progress), `timelineKey` and `timelineSortTs`, with no invented timestamps; pending messages order by the core's timeline time and merge with their acknowledged copy by client id. The core timeline identity switches from client to server id on ACK by design (`timeline_key_uses_server_id_after_ack`), while all four UI kits keep the client id as the live row key and retain the server id for authoritative actions and quote lookup. Vue previously used the changing core identity as its VNode key, destroying an already-open message menu during the ACK window; `MessageList.menu.test.ts` now pins the menu-and-row continuity and the live action refresh.
