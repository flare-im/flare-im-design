...
- **遗留（可选，非本批范围）**：批量式(非解耦)组件 CallControls/CallView/ContactDetail/ConversationDetails/Composer 等仍有硬编码中文默认文案（如「发消息」「免打扰」placeholder「发送消息…」）——四端皆然，属更大一轮"全组件去中文默认"（须四端同步做以免 parity 漂移），本批只覆盖解耦消息体 + composer 部件。
