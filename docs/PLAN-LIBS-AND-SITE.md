...
- [ ] **待续：Flutter/iOS/Compose shipped 补同款事件 + 图标参数**（本轮 Vue 先行 + 契约已立；三端按 contract-first 跟上：Flutter `onOpen/onDownload/onPlay/onTap`+`icon:Widget?` 包 GestureDetector；iOS 闭包 + Button；Compose lambda + clickable）。
