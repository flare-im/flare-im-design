...
## Anatomy
tab list, tab, selected indicator, panel. Regions use semantic tokens and host-owned content.

## Variants
fixed, scrollable, segmented. Choose one variant from task priority, not decoration.

## Sizes
compact, comfortable. Minimum interactive targets remain 48 logical pixels.

## States
default, hover, focus, selected, disabled. Loading preserves geometry; errors keep a recovery path.
