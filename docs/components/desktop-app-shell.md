...
## Overview
Compose navigation, primary/content/detail panes, commands, and overlays.

## When to use
Compose navigation, primary/content/detail panes, commands, and overlays.

## When not to use
Do not stretch a mobile route across a desktop window.

## Anatomy
navigation, primary pane, content, detail, overlay, command host. Regions use semantic tokens and host-owned content.

## Variants
single, dual, triple pane. Choose one variant from task priority, not decoration.

## Sizes
desktop and large desktop. Minimum interactive targets remain 48 logical pixels.

## States
detail hidden/inline/overlay, resizing, command open. Loading preserves geometry; errors keep a recovery path.

## Interactions
Components emit intent callbacks. The host owns data, permissions, persistence, and side effects.

## Keyboard
Tab follows panes; Escape closes context; documented global shortcuts are host-scoped.
