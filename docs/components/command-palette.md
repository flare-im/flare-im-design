...
## Overview
Find and invoke host-owned commands without memorizing navigation.

## When to use
Find and invoke host-owned commands without memorizing navigation.

## When not to use
Do not execute business operations or permission decisions inside the component.

## Anatomy
query, grouped results, selected command, description, shortcut. Regions use semantic tokens and host-owned content.

## Variants
default, compact. Choose one variant from task priority, not decoration.

## Sizes
bounded desktop, full-width compact. Minimum interactive targets remain 48 logical pixels.

## States
closed, idle, filtering, empty, busy. Loading preserves geometry; errors keep a recovery path.

## Interactions
Components emit intent callbacks. The host owns data, permissions, persistence, and side effects.

## Keyboard
ArrowUp/Down, Home/End, Enter, Escape; focus starts in query and restores.
