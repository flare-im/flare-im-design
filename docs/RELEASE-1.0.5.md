...
| 平台 | 包名 | 引入方式 |
|---|---|---|
| Web / Vue | `@flare-im/vue-ui` | `npm i @flare-im/vue-ui` |
| Web / tokens | `@flare-im/tokens` | `npm i @flare-im/tokens` |
| Web / 客户端 SDK | `@flare-im/sdk` | `npm i @flare-im/sdk` |
...
- 五端版本 `1.0.4` → `1.0.5`；`@flare-im/sdk` 由 `0.2.0` 一并对齐到 `1.0.5`。
...
- 解除 `@flare-im/sdk` 的 `private: true`，补 `description`。

## 依赖关系（决定发布顺序）

`@flare-im/vue-ui` 有 10 处**运行时值导入**指向 `@flare-im/sdk`
（`ConversationType`、`MessageContentType`、`uploadMediaInput` 等，不只是类型）。
该 SDK 不上 npm，vue-im-ui 发出去也是坏的。因此 npm 三个包的顺序是：

`@flare-im/sdk` → `@flare-im/tokens` → `@flare-im/vue-ui`
...
- **`@flare-im/vue-ui` 的 smoke.test.ts** 里有一处硬编码相对路径
  （`../../../../../@flare-im/sdk/src/adapter/codec/wireCodec.ts`），
