...
| `olderError` | `string \| null` |  | — | 历史加载错误，保留现有消息并允许显式重试。 |
| `loadOlderText` | `string` |  | — | 本地化加载/重试文案。 |
...
> [!TIP]
> 60fps 虚拟化；O(visible)；追加 / 前插时锚定滚动位。 toggle-select 目前只有 Vue 与 Flutter(onToggleSelect),iOS/Compose 尚未提供。locate-message 是 Vue 专有(工作台内跳转定位),原生端由宿主自行处理。

## 各端实现

<div class="flare-platform-grid">
  <div class="flare-platform-card"><h4>Vue</h4><div><code>FlareMessageList</code></div><div style="color:var(--vp-c-text-3);font-size:12px;margin-top:4px">@flare-im/vue-ui</div></div>
...
```vue [Vue]
<script setup>
import { FlareMessageList } from "@flare-im/vue-ui";
</script>
<template>
  <FlareMessageList
