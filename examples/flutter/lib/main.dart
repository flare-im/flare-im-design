...
FlareMessageData _msg(
  ...
  String text, {
  bool self = false,
  FlareMessageDeliveryStatus status = FlareMessageDeliveryStatus.read,
}) {
...
class _GalleryAppState extends State<GalleryApp> {
  // Start in dark mode so semantic surfaces can be inspected immediately;
  // the AppBar action toggles light and dark modes.
...
class _CommandPaletteDemo extends StatefulWidget {
  const _CommandPaletteDemo();

  @override
  State<_CommandPaletteDemo> createState() => _CommandPaletteDemoState();
}

class _CommandPaletteDemoState extends State<_CommandPaletteDemo> {
  bool _open = false;
  String _query = '';
  String _selectedId = 'new-message';
    ...
    FlareCommandPaletteGroup(
      ...
      commands: [
        FlareCommandPaletteCommand(
          id: 'new-message',
          label: '发起新会话',
          description: '选择联系人或群组',
          shortcut: 'Ctrl+N',
        ),
        FlareCommandPaletteCommand(
          id: 'search',
          label: '搜索消息',
          description: '在当前工作区中查找',
          shortcut: 'Ctrl+F',
        ),
        FlareCommandPaletteCommand(
          id: 'settings',
      ...
      child: Stack(
        ...
        children: [
          ...
          FlareCommandPalette(
            ...
            onSelectedIdChange: (value) => setState(() => _selectedId = value),
            onInvoke: (_) => setState(() => _open = false),
...
class _Gallery extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final moment = FlareMoment(
      ...
      images: const [
        FlareGridImage(alt: '设计稿一'),
        FlareGridImage(alt: '设计稿二'),
      ],
      location: '深圳·科技园',
      time: '刚刚',
      likes: const [
        FlareMomentLike(id: 'u1', name: 'Henry'),
        FlareMomentLike(id: 'u3', name: 'Kai'),
      ],
      comments: const [
        FlareMomentComment(
          id: 'c1',
          author: FlareMomentAuthor(id: 'u1', name: 'Henry'),
          text: '这个紫太顶了',
          time: '刚刚',
        ),
      ],
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          const _Section('CommandPalette', _CommandPaletteDemo()),
          _Section(
            'FlareScreen (brand)',
            SizedBox(
              // Tall enough for the whole card: at 200 it overflowed.
              height: 320,
              child: FlareScreen(
                title: '我的',
                surface: FlareScreenSurface.brand,
                scroll: false,
                child: Padding(
                  padding: const EdgeInsets.all(16),
                  // No-op callbacks so the gallery shows the actions.
          ...
          _Section(
            ...
            Row(
              children: const [
                FlareMessageStatus(status: FlareMessageDeliveryStatus.pending),
                SizedBox(width: 16),
                FlareMessageStatus(status: FlareMessageDeliveryStatus.sending),
                SizedBox(width: 16),
                FlareMessageStatus(status: FlareMessageDeliveryStatus.sent),
                SizedBox(width: 16),
                FlareMessageStatus(
                  status: FlareMessageDeliveryStatus.delivered,
                ),
                SizedBox(width: 16),
                FlareMessageStatus(status: FlareMessageDeliveryStatus.read),
                SizedBox(width: 16),
                FlareMessageStatus(status: FlareMessageDeliveryStatus.failed),
                SizedBox(width: 16),
                FlareMessageStatus(status: FlareMessageDeliveryStatus.retrying),
              ],
            ),
          ),
          const _Section(
          ...
          const _Section(
            'SearchBar',
            FlareSearchBar(placeholder: '搜索联系人、群组、消息'),
          ),
          _Section(
            'Button states',
            Wrap(
              ...
              runSpacing: 8,
              children: [
                FlareButton(label: '确认', onPressed: () {}),
          ...
          const _Section(
            'Input',
            FlareInput(placeholder: '输入内容', clearable: true),
          ),
          _Section(
            'Tabs',
            FlareFilterTabs(
              options: const [
                ...
                FlareFilterTabOption(value: 'unread', label: '未读', badge: 3),
              ],
              selected: 'all',
              onSelect: (_) {},
            ),
          ...
          const _Section(
            ...
            Wrap(
              ...
              runSpacing: 8,
              children: [
                FlareToast(message: '已保存', variant: FlareToastVariant.success),
                FlareToast(message: '正在同步', variant: FlareToastVariant.loading),
              ],
            ),
          ),
          const _Section(
            ...
            FlareSkeleton(
              variant: FlareSkeletonVariant.message,
              rows: 2,
              still: true,
            ),
          ),
          const _Section(
            ...
            FlareTransferProgress(
              name: '设计规范.pdf',
              state: FlareTransferState.transferring,
              statusText: '上传中 42%',
              progress: 0.42,
            ),
          ),
          const _Section(
            ...
            Column(
              children: [
                FlareStatusBanner(
                  text: '当前离线，消息将在恢复连接后发送',
                  tone: FlareStatusTone.neutral,
                ),
                SizedBox(height: 8),
                FlareStatusBanner(
                  text: '同步失败',
          ...
          _Section(
            'ConversationRow',
            FlareConversationRow(item: conversations.first),
          ),
          _Section(
            'ConversationList',
            SizedBox(
              height: 140,
              child: FlareConversationList(items: conversations),
            ),
          ),
          _Section('ContactItem', FlareContactItem(item: contacts[1])),
          _Section(
            'ContactList',
            SizedBox(height: 180, child: FlareContactList(items: contacts)),
          ),
          _Section(
            'GroupList',
            SizedBox(
              height: 120,
              child: FlareGroupList(
                items: [
          ...
          _Section(
            ...
            Column(
              ...
              children: [
                ...
                const SizedBox(height: 8),
                FlareMessageBubble(
                  message: _msg('text-in', 'Ivy Chen', '新版设计稿已经上传啦，帮忙看下～'),
                  currentUserId: 'me',
                ),
              ],
            ),
          ...
          _Section(
            ...
            Column(
              children: [
                FlareMessageBubble(
                  message: _msg('2', 'Ivy Chen', '新版设计稿已经上传啦～'),
                  currentUserId: 'me',
                  conversationKind: FlareConversationKind.group,
                ),
                FlareMessageBubble(
                  message: _msg('3', 'Me', '收到，下午看', self: true),
                  currentUserId: 'me',
                  conversationKind: FlareConversationKind.group,
                ),
              ],
          ...
          const _Section(
            'Reaction',
            FlareReactionSummary(
              reactions: [
                FlareReactionGroup(emoji: '👍', count: 3, reactedBySelf: true),
                FlareReactionGroup(emoji: '🎉', count: 2),
              ],
            ),
          ),
          _Section(
          ...
          const _Section(
            'SearchResult empty',
            FlareSearchResults(groups: [], query: 'missing'),
          ),
          _Section(
            'Composer',
            FlareComposer(
          ...
          const _Section(
            ...
            SizedBox(
              ...
              child: FlareAppLayout(
                primary: Center(child: Text('会话列表')),
                content: Center(child: Text('消息内容')),
                detail: Padding(
