<script setup>
// Lives INSIDE a <FlareUiProvider> so useFlareConfig() injects the live
// api. Shows how one root provider drives size + language + theme for every
// descendant kit component at once.
import { computed } from "vue";
import { useFlareConfig } from "@flare-im/vue-ui/composables";
import { useFlareAdaptive } from "@flare-im/vue-ui/composables";
import { FlareButton } from "@flare-im/vue-ui/components";
import { FlareIconButton } from "@flare-im/vue-ui/components";
import { FlareSelect } from "@flare-im/vue-ui/components";
...
.hint { margin: 0; font-size: 12px; color: var(--flare-color-primary-text); }
.seg { min-width: 34px; height: 30px; padding: 0 10px; border-radius: 8px; border: 1px solid var(--flare-color-border-primary); background: var(--flare-color-bg-secondary); color: var(--flare-color-text-secondary); font-size: 13px; cursor: pointer; transition: background-color 0.15s ease, border-color 0.15s ease, color 0.15s ease; }
...
.seg.on { background: var(--flare-color-bg-selected); border-color: var(--flare-color-primary); color: var(--flare-color-primary-text); font-weight: 500; }
