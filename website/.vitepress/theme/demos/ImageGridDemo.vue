<script setup>
import { ref } from "vue";
import { FlareImageGrid } from "@flare-im/vue-ui/components";
import DemoStage from "./DemoStage.vue";
// deterministic gradient placeholders (no external images)
// encodeURIComponent already turns `#` into `%23`; the old `.replace(/#/g, "%23")`
// ran first, so every colour and the `url(#g)` reference came out double-encoded
// (`%2523…`) and the gradient resolved to nothing.
