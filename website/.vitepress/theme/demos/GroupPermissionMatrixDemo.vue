...
import { FlareGroupPermissionMatrix } from "@flare-im/vue-ui/components";
...
const summary = computed(() => {
  ...
  const policy = s.joinPolicy ? { open: "公开", approval: "需审批", invite: "仅邀请" }[s.joinPolicy] : "未知";
  return [
    `加群 ${policy}`,
    s.muteAll ? "全员禁言" : "可发言",
  ...
  <DemoStage>
    <div class="gpm-demo">
      ...
      <div class="gpm-demo__controls">
        ...
        <button type="button" @click="settings = { ...settings, joinPolicy: null }">宿主不知道加群方式</button>
