...
function usePreset(name) {
  active.value = name;
  if (stage.value) applyFlareTheme(name, isDark.value ? "dark" : "light", stage.value);
}
onMounted(() => usePreset("violet"));
watch(isDark, () => usePreset(active.value));
  ...
  en.value
    ? {
        ...
        thread: [
          ...
          { id: 3, self: true, text: "Sure, I'll go through them this afternoon", time: "14:31", reactions: [{ emoji: "👍", count: 2, selected: true }] },
          { id: 4, self: true, text: "Direction looks right to me", time: "14:32", read: true },
          { id: 5, self: true, text: "One attachment still needs review", time: "14:33", failed: true },
        ],
      }
    : {
        ...
        thread: [
          ...
          { id: 3, self: true, text: "收到，我下午过一遍给你反馈", time: "14:31", reactions: [{ emoji: "👍", count: 2, selected: true }] },
          { id: 4, self: true, text: "整体方向没问题", time: "14:32", read: true },
          { id: 5, self: true, text: "还有一个附件需要复核", time: "14:33", failed: true },
        ],
      },
);
...
const bubbleThread = computed(() =>
  T.value.thread
    ...
    .map((m) => {
      ...
      const timestamp = new Date(2025, 0, 15, hours, minutes).getTime();
      return {
        row: {
        ...
        status: m.failed ? "failed" : m.read ? "read" : "sent",
        reactions: m.reactions,
        isRecalled: false,
        isRead: true,
        timelineKey: String(m.id),
        timelineSortTs: timestamp,
        attributes: {},
      },
      self: !!m.self,
      };
...
<template>
  <div class="showcase">
    <div class="chips">
      ...
      <button
        v-for="name in flareThemeNames" :key="name"
        class="chip" :class="{ on: active === name }"
        @click="usePreset(name)"
      >
        <span class="dot" :style="{ background: flareBuiltInThemes[name].light.colors.primary.default }" />{{ name }}
      </button>
    </div>
...
.chip { display: inline-flex; align-items: center; gap: 6px; padding: 4px 11px; border: 1px solid var(--vp-c-divider); border-radius: 5px; background: var(--vp-c-bg-soft); font-size: 12px; color: var(--vp-c-text-2); cursor: pointer; text-transform: capitalize; transition: border-color .15s, color .15s; }
.chip:hover { border-color: var(--vp-c-brand-1); }
.chip.on { border-color: var(--vp-c-brand-1); color: var(--vp-c-text-1); }
.dot { width: 10px; height: 10px; border-radius: 50%; }

.window { border: 1px solid var(--flare-color-border-primary); border-radius: 6px; overflow: hidden; background: var(--flare-color-bg-primary); }
...
.search { display: flex; align-items: center; gap: 7px; padding: 7px 10px; margin-bottom: 6px; border-radius: 6px; background: var(--flare-color-bg-tertiary); color: var(--flare-color-text-tertiary); font-size: 13px; }
.room { display: flex; align-items: center; gap: 10px; padding: 9px 10px; border: none; border-radius: 6px; background: none; cursor: pointer; text-align: left; }
.room.on { background: var(--flare-color-bg-selected); box-shadow: inset 3px 0 0 var(--flare-color-message-selected-border); }
...
.ht em { font-style: normal; font-size: 11px; color: var(--flare-color-success-text); }
...
.bubble.self { background: var(--flare-color-message-outgoing-background); border-color: var(--flare-color-message-outgoing-border); color: var(--flare-color-message-outgoing-foreground); border-radius: 16px 16px 4px 16px; align-items: flex-end; }
.meta { display: flex; align-items: center; gap: 3px; font-size: 11px; margin-top: 3px; color: var(--flare-color-text-tertiary); }
.bubble.self .meta { color: var(--flare-color-message-status-on-outgoing); }
...
.field { flex: 1; padding: 8px 12px; border: 1px solid var(--flare-color-focus-ring); border-radius: 6px; background: var(--flare-color-bg-secondary); color: var(--flare-color-text-tertiary); font-size: 13.5px; }
.send { width: 32px; height: 32px; border-radius: 50%; flex: none; background: var(--flare-color-primary); color: var(--flare-color-message-outgoing-foreground); display: flex; align-items: center; justify-content: center; }
