...
一份中立的 [`tokens.json`](https://github.com/flare-im/flare-im-design) 单一源，生成四端产物：Web `CSS`/`TS`、Dart、Swift、Kotlin。颜色、间距、字号、圆角完全一致，明暗双主题。下面的色板取自本站加载的**真实生成 CSS 变量**。
...
<div class="tok-grid">
  <div class="tok"><span class="sw" style="background:var(--flare-color-message-outgoing-background)"></span><code>bubble-self</code></div>
  <div class="tok"><span class="sw" style="background:var(--flare-color-message-incoming-background)"></span><code>bubble-other</code></div>
  <div class="tok"><span class="sw" style="background:var(--flare-color-bg-tertiary)"></span><code>bubble-robot</code></div>
  <div class="tok"><span class="sw" style="background:var(--flare-color-bg-tertiary)"></span><code>bubble-system</code></div>
</div>
...
.self { background: var(--flare-color-message-outgoing-background); }
```

```dart [Flutter]
final colors = FlareColors.of(Theme.of(context).brightness);
Container(color: colors.messageOutgoingBackground);
...
Box(Modifier.background(colors.messageOutgoingBackground))
```
...
.rbox { width: 56px; height: 56px; background: var(--flare-color-message-incoming-background); display: block; }
</style>
