...
- Place `FlareEmojiStickerPicker` inside the `media-panel` slot with `show-send-button=false`.
...
- `actions` configures ordering, icons, visibility, intent, and `enabled / disabledReason`. An empty array means no actions. Pages contain eight actions.
