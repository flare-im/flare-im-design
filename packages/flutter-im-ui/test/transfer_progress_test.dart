...
void main() {
  test('unknown progress stays unknown and terminal state is complete', () {
    ...
    expect(
      FlareTransferState.transferring.normalizedProgress(double.nan),
      isNull,
    );
    ...
    for (final scale in [1.0, 2.0]) {
      testWidgets('$state small phone text=$scale', (tester) async {
          ...
          MaterialApp(
            ...
            home: Scaffold(
              body: SingleChildScrollView(
                child: FlareTransferProgress(
                  name: '项目文档-长文件名-Quarterly-Report.pdf',
                  state: state,
                  statusText: '等待网络恢复，请稍后再试',
                  actionLabels: labels,
                  busy: busy,
                  onAction: (a) => picked = a,
                ),
        ...
        final expected = switch (state) {
          FlareTransferState.idle => <String>[],
          ...
          FlareTransferState.cancelled => ['retry'],
  ...
  testWidgets('status banner respects reduced motion and long action labels', (
    tester,
  ) async {
    tester.view.physicalSize = const Size(320, 900);
    tester.view.devicePixelRatio = 1;
