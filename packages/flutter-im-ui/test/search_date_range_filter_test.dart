...
void main() {
  group('dayStartMs / dayEndMs', () {
    test(
      'anchors a day to 00:00:00.000 and 23:59:59.999 of the given zone',
      () {
    ...
    test('spans exactly one inclusive day in a fixed-offset zone', () {
      for (final tz in [0, cst, est]) {
        expect(
          dayEndMs('2026-03-15', tz)! - dayStartMs('2026-03-15', tz)!,
          86399999,
        );
      }
    });
    ...
    test('uses the device zone when no offset is given', () {
      ...
      expect(
        [end.day, end.hour, end.minute, end.second, end.millisecond],
        [15, 23, 59, 59, 999],
      );
    });
  });
  ...
  group('datesFromRange / matchedOptionId / shouldOpenCustomRange', () {
    ...
    test('leaves an absent bound blank and re-reads another zone', () {
      expect(
        datesFromRange(const FlareSearchTimeRange(), cst),
        const FlareSearchDateDraft(from: '', to: ''),
      );
      expect(
        datesFromRange(
          FlareSearchTimeRange(fromTime: dayStartMs('2026-03-15', cst)),
          cst,
        ),
    ...
    test('matches presets by value, not by id', () {
      ...
      expect(
        matchedOptionId(const FlareSearchTimeRange(fromTime: 500), options),
        'week',
      );
      expect(matchedOptionId(const FlareSearchTimeRange(), options), 'all');
      expect(
        matchedOptionId(
          const FlareSearchTimeRange(fromTime: 1000, toTime: 2001),
          options,
        ),
        isNull,
      );
      expect(
        matchedOptionId(const FlareSearchTimeRange(toTime: 500), options),
        isNull,
      );
    });
    ...
    test('custom area opens only for an uncovered value or a forced host', () {
      ...
      expect(
        shouldOpenCustomRange(
          const FlareSearchTimeRange(fromTime: 7),
          options,
          false,
          true,
        ),
      ...
      expect(
        shouldOpenCustomRange(
          const FlareSearchTimeRange(fromTime: 7, toTime: 9),
          options,
          true,
        ),
        isTrue,
      ...
      expect(
        shouldOpenCustomRange(const FlareSearchTimeRange(), options, true),
        isFalse,
      );
    });
  });
  ...
  group('FlareSearchDateRangeFilter', () {
    ...
    Widget host(FlareSearchDateRangeFilter child) => MaterialApp(
    ...
    testWidgets('custom area opens for a value no preset covers', (
      tester,
    ) async {
      await tester.pumpWidget(
        host(
