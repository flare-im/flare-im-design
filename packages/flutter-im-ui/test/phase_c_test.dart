...
void main() {
    ...
    await tester.pumpWidget(
      _host(
        FlareNewFriendRequests(
          items: const [
            FlareFriendRequest(id: 'r1', name: 'Bob', message: 'hi'),
          ],
          acceptLabel: 'Accept',
  ...
  testWidgets('FlareGroupList renders member count', (tester) async {
    await tester.pumpWidget(
      _host(
        FlareGroupList(
          items: const [
            FlareGroupSummary(id: 'g1', name: 'Team', memberCount: 4),
          ],
  ...
  testWidgets('FlareProfilePanel + ProfileEditor + SettingsList construct', (
    tester,
  ) async {
  ...
  testWidgets('Call components render + hangup', (tester) async {
    ...
    await tester.tap(find.byIcon(flareIconMap['end-call']!));
    expect(hung, isTrue);
    ...
    await tester.pumpWidget(
      _host(
        const FlareCallView(
          peerName: 'Henry',
          mode: FlareCallMode.video,
          state: FlareCallState.connected,
          durationLabel: '02:14',
        ),
      ),
    );
    ...
    await tester.pumpWidget(
      _host(
        const FlareIncomingCall(callerName: 'Ivy', mode: FlareCallMode.audio),
      ),
    );
    expect(find.text(FlareStrings().incomingVoiceCall), findsOneWidget);
  });

  testWidgets('FlareResponsiveLayout: wide shows two panes, narrow single', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
  ...
  testWidgets('FlareMobileAppShell keeps bottom navigation under the content', (
    tester,
  ) async {
    await tester.pumpWidget(
      _host(
        FlareMobileAppShell(
          navigation: const [
            FlareNavigationGroup(
              id: 'main',
              items: [
                FlareNavigationItem(id: 'chat', label: '消息', icon: 'chats'),
                FlareNavigationItem(id: 'me', label: '我', icon: 'person'),
              ],
            ),
          ],
          activeNavigationId: 'chat',
    ...
    expect(
      tester.getTopLeft(find.text('消息')).dy,
      greaterThan(tester.getBottomLeft(find.text('内容')).dy),
    );
  });
}
