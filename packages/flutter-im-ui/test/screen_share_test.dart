...
void main() {
  ...
  test(
    'a viewer can never stop, and an unavailable runtime offers nothing',
    () {
      final viewing = screenShareActions(
        FlareScreenShareState.viewing,
        hasStart: true,
        hasStop: true,
        hasCancel: true,
  ...
  test('tone and icon per state', () {
    expect(
      screenShareTone(FlareScreenShareState.idle),
      FlareStatusTone.neutral,
    );
    expect(
      screenShareTone(FlareScreenShareState.requesting),
      FlareStatusTone.warning,
    );
    expect(
      screenShareTone(FlareScreenShareState.sharing),
      FlareStatusTone.success,
    );
    expect(
      screenShareTone(FlareScreenShareState.viewing),
      FlareStatusTone.info,
    );
    expect(
      screenShareTone(FlareScreenShareState.unavailable),
      FlareStatusTone.neutral,
    );
    expect(FlareScreenShareState.values.map(screenShareIconName).toList(), [
      'screen-share',
      'refresh',
      'video',
      'eye',
    ...
    for (final state in FlareScreenShareState.values) {
      expect(flareIconNames, contains(screenShareIconName(state)));
    }
  });

  Widget host(FlareScreenShare child) => MaterialApp(
  ...
  testWidgets(
    'requesting shows progress and cancel; viewing and unavailable have no action',
    (tester) async {
      var cancels = 0;
      await tester.pumpWidget(
        host(
