...
void main() {
  ...
  List<FlareRelationAction> ids(
    FlareRelationState r,
    FlareRelationCapabilities? c,
  ...
  group('relationActions', () {
    ...
    test('pendingOut never re-offers add, only block', () {
      expect(ids(FlareRelationState.pendingOut, all), [
        FlareRelationAction.block,
      ]);
      expect(
        ids(
          FlareRelationState.pendingOut,
          const FlareRelationCapabilities(add: true),
        ),
        isEmpty,
      );
      expect(relationShowsPending(FlareRelationState.pendingOut), isTrue);
      for (final r in FlareRelationState.values.where(
        (r) => r != FlareRelationState.pendingOut,
      )) {
    ...
    test('pendingIn offers accept as primary, then reject and block', () {
      expect(ids(FlareRelationState.pendingIn, all), [
        FlareRelationAction.accept,
        FlareRelationAction.reject,
        FlareRelationAction.block,
      ]);
      final entries = relationActions(FlareRelationState.pendingIn, all);
      expect(entries.where((e) => e.primary).map((e) => e.action), [
        FlareRelationAction.accept,
      ]);
    ...
    test(
      'friends offers message as primary and marks remove and block destructive',
      () {
        expect(ids(FlareRelationState.friends, all), [
          FlareRelationAction.message,
          FlareRelationAction.remove,
          FlareRelationAction.block,
        ]);
    ...
    test('blocked offers unblock and nothing else', () {
      expect(ids(FlareRelationState.blocked, all), [
        FlareRelationAction.unblock,
      ]);
      expect(
        relationActions(FlareRelationState.blocked, all).first.primary,
        isTrue,
      );
    });

    test(
      'a missing capability removes just its own entry and keeps the order',
      () {
        expect(
          ids(
            FlareRelationState.none,
            const FlareRelationCapabilities(block: true),
          ),
          [FlareRelationAction.block],
        );
        expect(
  ...
  testWidgets('error stays on screen and only dismiss clears it', (
    tester,
  ) async {
    var dismissed = 0;
    await tester.pumpWidget(
  ...
  testWidgets('no capability shows the empty text, not a blank bar', (
    tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
        home: Scaffold(
          body: FlareRelationActionBar(relation: FlareRelationState.none),
        ),
      ),
    );
