...
void main() {
  group('paneRender', () {
    ...
    test('degrades a missing state / default status to the host content', () {
      ...
      expect(
        paneRender(const FlareWorkspacePaneState(message: '只有文案')),
        FlareWorkspacePaneRender.content,
      );
    });
  ...
  group('paneRetryVisible', () {
    ...
    test(
      'a failure without a handler or without a label shows reason only',
      () {
  ...
  group('pane independence', () {
    test('a failing pane leaves the loaded and loading panes untouched', () {
      ...
      const detail = FlareWorkspacePaneState(
        status: FlareWorkspacePaneStatus.loading,
      );
  ...
  group('workspaceBannerActionVisible', () {
    test('needs a visible banner + non-blank label + host handler', () {
      const withLabel = FlareWorkspaceBanner(message: '离线', actionLabel: '重连');
      ...
      expect(
        workspaceBannerActionVisible(
          const FlareWorkspaceBanner(message: '离线'),
          true,
        ),
        isFalse,
      );
  ...
  test('workspaceBannerTone maps error onto danger and defaults to info', () {
    expect(FlareWorkspaceBannerTone.values.map(workspaceBannerTone).toList(), [
  ...
  test('paneSkeletonVariant gives rows / bubbles / card per pane', () {
    expect(FlareWorkspacePaneKey.values.map(paneSkeletonVariant).toList(), [
  ...
  testWidgets(
    'failure names the reason and retries only with label + handler',
    (tester) async {
  ...
  testWidgets('a blank banner message renders no banner strip', (tester) async {
    await tester.pumpWidget(
      host(banner: const FlareWorkspaceBanner(message: '   ')),
    );
    await tester.pump();
  ...
  testWidgets(
    'detail pane still renders its state when the host passed no detail slot',
    (tester) async {
