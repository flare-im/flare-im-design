...
void main() {
  const caps = FlareConversationBatchCapabilities(
    markRead: true,
    mute: true,
    delete: true,
  );
  ...
  group('batchActionsAvailable', () {
    test('canonical order, capability filtered', () {
      expect(batchActionsAvailable(['a', 'b'], caps, false), [
        FlareConversationBatchAction.markRead,
        FlareConversationBatchAction.mute,
        FlareConversationBatchAction.delete,
      ]);
      expect(
        batchActionsAvailable(
          ['a'],
          const FlareConversationBatchCapabilities(delete: true, archive: true),
          false,
        ),
        [
          FlareConversationBatchAction.archive,
          FlareConversationBatchAction.delete,
        ],
      );
    });
    test(
      'empty when nothing selected, busy, no capabilities or over the limit',
      () {
        ...
        expect(
          batchActionsAvailable(['a', 'b', 'c'], caps, false, 0),
          hasLength(3),
        );
      },
    );
  ...
  group('summarizeBatchResult', () {
    test('counts both outcomes and deduplicates retry ids', () {
      const result = FlareConversationBatchResult(
        succeeded: ['a', 'b'],
        failed: [
  ...
  testWidgets('empty selection disables actions and shows the empty hint', (
    tester,
  ) async {
  ...
  testWidgets(
    '320 wide: actions wrap, delete emits ids, partial failure expands and retries',
    (tester) async {
  ...
  testWidgets('busy disables everything and over-limit shows the limit hint', (
    tester,
  ) async {
