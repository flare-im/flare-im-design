...
void main() {
  group('unknownUserPresentation', () {
    ...
    test('carries a tone alongside the icon, never instead of it', () {
      expect(
        unknownUserPresentation(FlareUnknownUserKind.unknown).tone,
        FlareUnknownUserTone.neutral,
      );
      expect(
        unknownUserPresentation(FlareUnknownUserKind.deactivated).tone,
        FlareUnknownUserTone.neutral,
      );
      expect(
        unknownUserPresentation(FlareUnknownUserKind.blocked).tone,
        FlareUnknownUserTone.danger,
      );
      expect(
        unknownUserPresentation(FlareUnknownUserKind.unreachable).tone,
  ...
  group('shortenUserId', () {
    ...
    test(
      'honours a custom budget and floors it so the result stays readable',
      () {
  ...
  testWidgets('row density shows the kind title, not the raw id', (
    tester,
  ) async {
  ...
  testWidgets('every kind renders its own default title', (tester) async {
    Future<void> check(FlareUnknownUserKind kind, String expected) async {
      await tester.pumpWidget(
        MaterialApp(
          home: Scaffold(
            body: FlareUnknownUserPlaceholder(userId: 'u1', kind: kind),
          ),
        ),
      );
  ...
  testWidgets('card density shows host detail and stays action-free', (
    tester,
  ) async {
    await tester.pumpWidget(
      const MaterialApp(
  ...
  testWidgets(
    'a blank id hides the diagnostic line instead of showing an empty one',
    (tester) async {
      await tester.pumpWidget(
        const MaterialApp(
          home: Scaffold(
            body: FlareUnknownUserPlaceholder(
              userId: '   ',
              kind: FlareUnknownUserKind.unknown,
            ),
          ),
        ),
      );
