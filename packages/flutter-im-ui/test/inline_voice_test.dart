...
void main() {
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(find.byTooltip(_strings.inlineVoiceStart));
      await tester.pump();
      await settleIO(tester);
      capture.data.add(Uint8List(32000));
      await tester.pump(const Duration(milliseconds: 200));
      await tester.tap(find.byTooltip(_strings.inlineVoicePause));
      await waitFor(
        tester,
        () => find.byTooltip(_strings.inlineVoiceResume).evaluate().isNotEmpty,
      );
      expect(capture.pauses, 1);
      expect(
        find.byTooltip(_strings.inlineVoiceResume),
        findsOneWidget,
        reason: tester
            .widgetList<Text>(find.byType(Text))
            .map((t) => t.data)
      ...
      await tester.tap(find.byTooltip(_strings.inlineVoiceResume));
      await tester.pump();
      await settleIO(tester);
      capture.data.add(Uint8List(32000));
      await tester.pump(const Duration(milliseconds: 200));
      ...
      expect(attempts, 1);
      expect(keyboard, 0);
      // The failure is said in the strings table's words, without an
      // exception type in front of them.
      expect(find.text(_strings.inlineVoiceSendFailed), findsOneWidget);
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.tap(find.byTooltip(_strings.inlineVoiceStart));
      await tester.pump();
      await tester.tap(find.byTooltip(_strings.inlineVoiceKeyboard));
      await tester.pump();
