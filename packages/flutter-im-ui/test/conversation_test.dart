...
void main() {
  group('FlareConversationRow', () {
    ...
    testWidgets('renders unread badge, caps at 999+', (tester) async {
      await tester.pumpWidget(
        _host(FlareConversationRow(item: _row('c1', 'A', unread: 1200))),
      );
    ...
    testWidgets('draft takes precedence over preview', (tester) async {
      ...
      // The prefix comes from FlareStrings, like every other inline string —
      // it used to be a hardcoded '[Draft] ' that no host could translate.
      expect(
        find.textContaining(const FlareStrings().conversationRowDraft.trim()),
        findsOneWidget,
      );
      expect(find.textContaining('wip'), findsOneWidget);
    });

    testWidgets('a host can still override the draft prefix', (tester) async {
      await tester.pumpWidget(
        _host(
    ...
    testWidgets('previewSpansBuilder is ignored while a draft exists', (
      ...
      await tester.pumpWidget(
        _host(
          FlareConversationRow(
            item: _row('c1', 'A', draft: 'wip'),
            previewSpansBuilder: (context, base) => const [
              TextSpan(text: 'should not show'),
            ],
          ),
        ),
      );
  ...
  group('FlareConversationList', () {
    testWidgets('empty shows placeholder', (tester) async {
      ...
      expect(find.text(const FlareStrings().noConversations), findsOneWidget);
    });
