...
class _FlareEmojiPackMessageState extends State<FlareEmojiPackMessage> {
  ...
  Widget build(BuildContext context) {
    ...
    if (packKey != null) {
      final label = FlareEmojiStickerCatalog.instance.emojiBracketLabel(
        packKey,
        locale: locale,
      );
      return SizedBox(
...
List<FlarePlainTextEmojiSegment> splitPlainTextForEmojiDisplay(String text) {
  ...
  for (final m in _kEmojiToken.allMatches(text)) {
    ...
    final key = m.group(1)!;
    out.add(
      catalog.hasEmojiKey(key)
          ? FlarePlainEmojiPack(key)
          : FlarePlainEmojiUnknown(key),
    );
    cursor = m.end;
  }
