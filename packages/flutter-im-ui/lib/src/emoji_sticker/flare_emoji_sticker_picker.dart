...
class FlareEmojiStickerPicker extends StatefulWidget {
  ...
  @override
  State<FlareEmojiStickerPicker> createState() =>
      _FlareEmojiStickerPickerState();
}
...
class _FlareEmojiStickerPickerState extends State<FlareEmojiStickerPicker> {
  ...
  Widget _tabBar(
    FlareColors colors,
    List<FlareStickerManifestPack> packs,
    int tab,
  ) {
    final labels = <String>[
      widget.emojiLabel ?? FlareStrings.of(context).emojiPickerEmoji,
      ...packs.map((p) => p.title),
    ];
    return Container(
      height: 44,
