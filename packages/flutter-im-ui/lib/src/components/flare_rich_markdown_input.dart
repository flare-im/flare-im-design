...
class FlareRichMarkdownLabels {
  const FlareRichMarkdownLabels({
    this.bold = '加粗',
    this.italic = '斜体',
    this.code = '代码',
    this.list = '列表',
...
class FlareRichMarkdownInput extends StatefulWidget {
  const FlareRichMarkdownInput({
    ...
    this.maxLength,
    this.placeholder,
    this.labels = const FlareRichMarkdownLabels(),
    this.onChanged,
    this.onSubmit,
  });
  ...
  final FlareRichMarkdownLabels labels;
...
class _FlareRichMarkdownInputState extends State<FlareRichMarkdownInput> {
  ...
  Widget _formatBar(FlareColors colors) {
    ...
    return Row(
      children: [
        btn(Icons.format_bold_rounded, () => _wrap('**'), widget.labels.bold),
        btn(
          Icons.format_italic_rounded,
          () => _wrap('*'),
          widget.labels.italic,
        ),
        btn(Icons.code_rounded, () => _wrap('`'), widget.labels.code),
        btn(
          Icons.format_list_bulleted_rounded,
          () => _wrap('\n- ', ''),
          widget.labels.list,
        ),
        btn(Icons.link_rounded, () => _wrap('[', '](url)'), widget.labels.link),
      ],
    );
  }
}
