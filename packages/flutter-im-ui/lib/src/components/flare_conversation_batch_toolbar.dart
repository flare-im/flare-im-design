...
class FlareConversationBatchFailure {
  const FlareConversationBatchFailure({
    required this.id,
    required this.title,
...
class FlareConversationBatchResult {
  const FlareConversationBatchResult({
...
class FlareConversationBatchSummary {
  const FlareConversationBatchSummary({
...
List<FlareConversationBatchAction> batchActionsAvailable(
  ...
  if (busy ||
      selectedIds.isEmpty ||
      batchSelectionExceeded(selectedIds.length, maxSelection))
    return const [];
  if (capabilities == null) return const [];
  return FlareConversationBatchAction.values
      .where(capabilities.allows)
      .toList(growable: false);
}

/// Counts of the previous batch and the deduplicated IDs a retry should target.
FlareConversationBatchSummary summarizeBatchResult(
  FlareConversationBatchResult? result,
) {
  if (result == null)
    return const FlareConversationBatchSummary(
      succeededCount: 0,
      failedCount: 0,
      retryIds: [],
    );
...
class FlareConversationBatchToolbar extends StatefulWidget {
  ...
  final void Function(FlareConversationBatchAction action, List<String> ids)?
  onAction;
  ...
  @override
  State<FlareConversationBatchToolbar> createState() =>
      _FlareConversationBatchToolbarState();
}

/// Sentinel key for the retry-failed button in the pending tracker.
const _retryKey = Object();

class _FlareConversationBatchToolbarState
    extends State<FlareConversationBatchToolbar> {
  Object? _pending;
  ...
  Widget build(BuildContext context) {
    ...
    final available = batchActionsAvailable(
      widget.selectedIds,
      widget.capabilities,
      widget.busy,
      widget.maxSelection,
    ...
    final hint = widget.busy
        ...
        : null;
    final visible = FlareConversationBatchAction.values
        .where(widget.capabilities.allows)
        .toList();
                      ...
                      Semantics(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            if (widget.busy || exceeded)
                              const SizedBox(width: 6),
                            Text(
                              hint,
                              style: TextStyle(
                                fontSize: FlareSizes.fontSizeSm,
                                color: exceeded && !widget.busy
                                    ? colors.warning
                                    : colors.textTertiary,
                              ),
                            ),
                    ...
                    for (final action in visible)
                      if (widget.onAction != null)
                        _button(
                          ...
                          color: action == FlareConversationBatchAction.delete
                              ? colors.error
                              : null,
                          onTap: () {
  ...
  Widget _resultStrip(
    FlareColors colors,
    FlareConversationBatchSummary summary,
  ) {
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              children: [
                      ...
                      child: Text.rich(
                        TextSpan(
                          children: [
                            ...
                            if (failed && summary.succeededCount > 0)
                              const TextSpan(text: ' · '),
                            if (summary.succeededCount > 0 || !failed)
                              TextSpan(
                                text: _fill(
                                  widget.succeededSummaryText,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (summary.retryIds.isNotEmpty &&
                        widget.onRetryFailed != null)
                      _button(
                        colors,
                        icon: Icons.refresh,
                        label:
                            '${widget.retryFailedText} (${summary.retryIds.length})',
                        enabled: !widget.busy,
          ...
          if (_expanded &&
              widget.result != null &&
              widget.result!.failed.isNotEmpty)
            Padding(
  ...
  IconData _icon(FlareConversationBatchAction a) => flareIconGlyph(switch (a) {
    FlareConversationBatchAction.markRead => 'read',
    FlareConversationBatchAction.mute => 'mute',
    FlareConversationBatchAction.archive => 'archive',
    FlareConversationBatchAction.delete => 'delete',
  });
  ...
  Widget _button(
    ...
    final fg = primary
        ? Colors.white
        : (enabled || pending
              ? (color ?? (ghost ? colors.textSecondary : colors.textPrimary))
              : colors.textTertiary);
    final bg = primary
        ? colors.primary
        : (ghost ? Colors.transparent : colors.bgSecondary);
    return Semantics(
      button: true,
      enabled: enabled,
          ...
          child: Container(
            ...
            child: Row(
              ...
              children: [
                ...
                Text(
                  label,
                  style: TextStyle(color: fg, fontSize: FlareSizes.fontSizeMd),
                  softWrap: false,
                ),
              ],
            ),
