...
class FlareConversationList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (items.isEmpty) {
      ...
      return Center(child: emptyPlaceholder ?? const _DefaultEmpty());
    }
    ...
    return NotificationListener<ScrollNotification>(
      ...
      child: ListView.builder(
        findChildIndexCallback: (key) {
          final index = ordered.indexWhere((item) => ValueKey(item.id) == key);
...
class _DefaultEmpty extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Text(
        FlareStrings.of(context).noConversations,
        style: TextStyle(
