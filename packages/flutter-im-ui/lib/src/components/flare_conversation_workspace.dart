...
class FlareWorkspacePaneState {
  ...
  @override
  int get hashCode => Object.hash(status, message, description, actionLabel);

  @override
  String toString() =>
      'FlareWorkspacePaneState(status: $status, message: $message, description: $description, actionLabel: $actionLabel)';
}
...
bool paneEmptyActionVisible(FlareWorkspacePaneState? state, bool hasAction) {
...
bool workspaceBannerActionVisible(
...
///
/// Both [FlareConversationWorkspace] and [FlareWorkspaceFrame] render their
/// panes through it, so the inbox and a settings surface fail the same way
/// instead of two dialects of "something went wrong".
class FlareWorkspacePaneView extends StatelessWidget {
  const FlareWorkspacePaneView({
    super.key,
    required this.child,
    this.state = const FlareWorkspacePaneState(),
    this.skeleton = FlareSkeletonVariant.conversation,
    this.skeletonRows = 6,
    this.emptyIcon = 'chats',
    this.emptyText = '',
    this.failureText = '',
    this.loadingText = '',
    this.onRetry,
  ...
  final Widget child;
  final FlareWorkspacePaneState state;
  final FlareSkeletonVariant skeleton;
  final int skeletonRows;

  /// A semantic icon name from `flareIconNames` for the empty state.
        ...
        return Semantics(
          ...
          child: Padding(
            ...
            child: Align(
              ...
              child: FlareSkeleton(variant: skeleton, rows: skeletonRows),
            ),
          ),
        );
      case FlareWorkspacePaneRender.empty:
        final action = paneEmptyActionVisible(state, onEmptyAction != null);
        return Center(
          child: SingleChildScrollView(
            child: FlareEmptyState(
              ...
              icon: emptyIcon,
              actionText: action ? state.actionLabel : null,
              onAction: action ? onEmptyAction : null,
            ),
          ),
        );
...
class FlareConversationWorkspace extends StatelessWidget {
  ...
  /// The panes in use once the width is known ([FlareResponsiveLayout.onLayoutChange]).
  ...
  /// The next step an empty pane offers. Without it no button is shown.
  final ValueChanged<FlareWorkspacePaneKey>? onEmptyAction;
  ...
  String _emptyIcon(FlareWorkspacePaneKey pane) => switch (pane) {
  ...
  }) => FlareWorkspacePaneView(
    state: state,
    skeleton: paneSkeletonVariant(key),
    skeletonRows: skeletonRows,
    emptyIcon: _emptyIcon(key),
    emptyText: emptyText,
    failureText: failureText,
    loadingText: loadingText,
    onRetry: onRetry == null ? null : () => onRetry!.call(key),
    onEmptyAction: onEmptyAction == null
        ? null
        : () => onEmptyAction!.call(key),
    child: content,
  );
