...
class FlareRelationActionEntry {
  const FlareRelationActionEntry(
    this.action, {
    this.primary = false,
...
class _FlareRelationActionBarState extends State<FlareRelationActionBar> {
  String get _addText =>
      widget.addText ?? FlareStrings.of(context).relationActionBarAdd;
  String get _acceptText =>
      widget.acceptText ?? FlareStrings.of(context).relationActionBarAccept;
  String get _rejectText =>
      widget.rejectText ?? FlareStrings.of(context).reject;
  String get _removeText =>
      widget.removeText ?? FlareStrings.of(context).relationActionBarRemove;
  String get _blockText =>
      widget.blockText ?? FlareStrings.of(context).relationActionBarBlock;
  String get _unblockText =>
      widget.unblockText ?? FlareStrings.of(context).relationActionBarUnblock;
  String get _messageText =>
      widget.messageText ?? FlareStrings.of(context).sendMessage;
  String get _pendingText =>
      widget.pendingText ?? FlareStrings.of(context).relationActionBarPending;
  String get _busyText =>
      widget.busyText ?? FlareStrings.of(context).relationActionBarBusy;
  String get _dismissErrorText =>
      widget.dismissErrorText ??
      FlareStrings.of(context).relationActionBarDismissError;
  String get _emptyText =>
      widget.emptyText ?? FlareStrings.of(context).relationActionBarEmpty;
  ...
  String _label(FlareRelationAction action) => switch (action) {
    FlareRelationAction.add => _addText,
    FlareRelationAction.accept => _acceptText,
    FlareRelationAction.reject => _rejectText,
    FlareRelationAction.remove => _removeText,
    FlareRelationAction.block => _blockText,
    FlareRelationAction.unblock => _unblockText,
    FlareRelationAction.message => _messageText,
  };
              ...
              children: [
                ...
                if (danger.isNotEmpty)
                  Container(
                    ...
                    decoration: BoxDecoration(
                      border: Border(
                        left: BorderSide(color: colors.borderSecondary),
                      ),
                    ),
                    child: Wrap(
                      crossAxisAlignment: WrapCrossAlignment.center,
                      ...
                      runSpacing: FlareSizes.spacingSm,
                      children: [
                        for (final entry in danger) _button(colors, entry),
                      ],
                    ),
                  ),
              ],
  ...
  Widget _errorStrip(FlareColors colors, String error) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Icon(Icons.error_outline, size: 16, color: colors.errorText),
  ...
  Widget _button(FlareColors colors, FlareRelationActionEntry entry) {
    ...
    return Semantics(
      button: true,
      enabled: enabled,
      label: pending ? '$label · ${_busyText}' : label,
      child: Opacity(
