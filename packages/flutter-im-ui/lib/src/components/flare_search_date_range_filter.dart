...
FlareSearchTimeRange? rangeFromDates(
  String from,
  String to, [
  int? tzOffsetMinutes,
]) {
...
FlareSearchDateDraft datesFromRange(
  FlareSearchTimeRange range, [
  int? tzOffsetMinutes,
...
class _FlareSearchDateRangeFilterState
  ...
  String get _title => widget.title ?? FlareStrings.of(context).timeRange;
  String get _customText =>
      widget.customText ?? FlareStrings.of(context).searchDateRangeFilterCustom;
  String get _fromLabel =>
      widget.fromLabel ?? FlareStrings.of(context).searchDateRangeFilterFrom;
  String get _toLabel =>
      widget.toLabel ?? FlareStrings.of(context).searchDateRangeFilterTo;
  String get _clearText => widget.clearText ?? FlareStrings.of(context).clear;
  String get _unlimitedText =>
      widget.unlimitedText ??
      FlareStrings.of(context).searchDateRangeFilterUnlimited;
  String get _invalidText =>
      widget.invalidText ??
      FlareStrings.of(context).searchDateRangeFilterInvalid;
  late FlareSearchDateDraft _draft = datesFromRange(
    widget.value,
    widget.tzOffsetMinutes,
  );
  ...
  void _chooseOption(FlareSearchRangeOption option) {
    ...
    final next = FlareSearchTimeRange(
  ...
  String _summary() {
    if (unrestrictedRange(widget.value)) return _unlimitedText;
    ...
    final parts = <String>[
      if (dates.from.isNotEmpty) '${_fromLabel} ${dates.from}',
      if (dates.to.isNotEmpty) '${_toLabel} ${dates.to}',
    ];
    return parts.isEmpty ? _unlimitedText : parts.join(' · ');
  }
