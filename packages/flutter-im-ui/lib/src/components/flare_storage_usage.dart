...
bool _isKnownBytes(num? value) => value != null && value.isFinite && value >= 0;
...
double? storageShare(num? bytes, num? total) {
  if (!_isKnownBytes(bytes) || !_isKnownBytes(total) || total! <= 0)
    return null;
...
class FlareStorageUsage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalLabel = _totalIsFloor(totals)
        ? '$atLeastText $totalValue'
        : totalValue;
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          child: Row(
            mainAxisSize: MainAxisSize.min,
            children: [
              Icon(Icons.delete_outline, size: 14, color: colors.errorText),
