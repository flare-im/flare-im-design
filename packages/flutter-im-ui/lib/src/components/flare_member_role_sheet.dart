...
List<FlareMemberRoleActionEntry> memberRoleActions(
  ...
  if (capabilities.transferOwner && viewerRole == FlareGroupMemberRole.owner) {
    out.add(
      const FlareMemberRoleActionEntry(
...
/// ownership. Owns no positioning — present it with `FlareBottomSheet.show` or
/// in a popover. It only emits intent: the second confirmation for remove /
...
class _FlareMemberRoleSheetState extends State<FlareMemberRoleSheet> {
  ...
  void didUpdateWidget(FlareMemberRoleSheet oldWidget) {
    ...
    if (widget.member.id != oldWidget.member.id || widget.busy)
      _muteOpen = false;
  }

  // Promote and demote share the admin glyph, mute and unmute the silence
  // glyph; the label says which way each one goes.
  static IconData _iconFor(FlareMemberRoleAction action) => flareIconGlyph(
    switch (action) {
  ...
  Widget build(BuildContext context) {
    ...
    final entries =
        memberRoleActions(widget.member, widget.viewerRole, widget.capabilities)
            // mute needs host durations; with none supplied the row is not offered.
            .where(
              (e) =>
                  e.action != FlareMemberRoleAction.mute ||
                  widget.muteDurations.isNotEmpty,
            )
            .toList();
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (primary.isNotEmpty)
                    _group(primary, colors, danger: false),
                  ...
                  children: [
                    Container(
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color:
                              widget.member.role == FlareGroupMemberRole.member
          ...
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                if (expandable)
                  FlareIcon(
                    _muteOpen ? 'chevron-up' : 'chevron-down',
