...
final class RelationActionBarTests: XCTestCase {
    ...
    func testNoneOffersAddAsPrimaryPlusBlock() {
        XCTAssertEqual(ids(FlareRelationState.none, all), [.add, .block])
        let entries = relationActions(FlareRelationState.none, capabilities: all)
    ...
    func testMissingCapabilityRemovesOnlyItsOwnEntry() {
        XCTAssertEqual(ids(FlareRelationState.none, .init(block: true)), [.block])
        XCTAssertEqual(ids(FlareRelationState.none, .init(add: true)), [.add])
