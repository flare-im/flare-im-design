...
public struct Contact: Identifiable, Sendable {
    ...
    /// The person's public Flare ID (the handle they chose), shown by ContactDetail and ProfileCard only when
    /// set. Never pass the internal account id (`id`) here: it is not shown.
    ...
    public init(id: String, name: String, avatarURL: String? = nil, signature: String? = nil,
                presence: FlarePresence? = nil, indexKey: String? = nil, flareId: String? = nil,
        ...
        self.presence = presence; self.indexKey = indexKey; self.flareId = flareId
...
/// Whether a friend request came to the user or was sent by the user.
...
public struct FriendRequest: Identifiable, Sendable {
    ...
    /// Incoming requests are accepted or declined; outgoing ones wait (and can be withdrawn).
...
/// What a settings row *is*, which decides what it announces and whether it can be operated
/// (FR-084, the Vue contract `FlareSettingKind`):
/// - `navigation`: opens a page or a picker — a button with a chevron;
/// - `toggle`: a switch;
/// - `action`: runs in place (clear history, mark unread, sign out) — a button, no chevron, `danger`
///   for a destructive one;
/// - `value`: read-only information — not a control at all: no button, no chevron, taps do nothing,
///   and assistive technology reads "label, detail" as one element.
public enum FlareSettingKind: Sendable { case navigation, toggle, action, value }

public struct FlareSettingsItem: Identifiable, Sendable {
    public var id: String { key }
    public let key: String
    public let label: String
    /// Semantic kit icon name (``flareIconNames``), drawn through the registry; an unknown name draws
    /// the registry's fallback glyph.
    ...
    public init(key: String, label: String, icon: String? = nil,
        ...
        self.key = key; self.label = label; self.icon = icon
