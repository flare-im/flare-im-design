...
public struct MomentsVisibilityRuleListView: View {
    ...
    private var copy: MomentsVisibilityLabels.Resolved { labels.resolve(strings) }
        ...
        VStack(alignment: .leading, spacing: 0) {
            HStack(alignment: .top, spacing: FlareSizes.spacingSm) {
                Image(systemName: isHideFrom ? "eye.slash" : "speaker.slash")
                    ...
                    .foregroundColor(isHideFrom ? colors.warningText : colors.textTertiary)
                    .padding(.top, 2)
                VStack(alignment: .leading, spacing: 2) {
                    Text(isHideFrom ? copy.hideFromTitle : copy.muteTitle)
                        .font(.system(size: FlareSizes.fontSizeLg))
                        .foregroundColor(colors.textPrimary)
                    Text(isHideFrom ? copy.hideFromHint : copy.muteHint)
                        .font(.system(size: FlareSizes.fontSizeSm))
                        .foregroundColor(colors.textTertiary)
                }
                Spacer()
                Button { onAdd?() } label: {
                    Image(systemName: flareIconSymbol("person-add"))
                        .foregroundColor(colors.textSecondary)
                        .flareTouchTarget()
                }
                .buttonStyle(.plain)
                .flareCompactLayout(width: FlareSizes.iconSizeMd, height: FlareSizes.iconSizeMd)
...
public struct MomentsVisibilityLabels: Sendable {
    ...
    /// Name of the add-person key.
    public let add: String?

    public init(hideFromTitle: String? = nil,
                hideFromHint: String? = nil,
                muteTitle: String? = nil,
                muteHint: String? = nil,
                empty: String? = nil,
                remove: String? = nil,
                add: String? = nil) {
        ...
        self.empty = empty; self.remove = remove; self.add = add
    }
}

public extension MomentsVisibilityLabels {
    ...
    func resolve(_ strings: FlareStrings) -> Resolved {
        Resolved(
            hideFromTitle: hideFromTitle ?? strings.momentsVisibilityRuleListHideFromTitle,
            hideFromHint: hideFromHint ?? strings.momentsVisibilityRuleListHideFromHint,
            muteTitle: muteTitle ?? strings.momentsVisibilityRuleListMuteTitle,
            muteHint: muteHint ?? strings.momentsVisibilityRuleListMuteHint,
            empty: empty ?? strings.momentsVisibilityRuleListEmpty,
            remove: remove ?? strings.momentsVisibilityRuleListRemove,
            add: add ?? strings.momentsVisibilityRuleListAdd
        )
    }
}
...
public struct ContactMatchListView: View {
    ...
    private var copy: ContactMatchLabels.Resolved { labels.resolve(strings) }
...
public extension ContactMatchLabels {
    /// Every label filled in: an explicit label wins, otherwise the `flareStrings` provider.
    struct Resolved: Sendable {
        public let add: String
        public let message: String
    ...
    func resolve(_ strings: FlareStrings) -> Resolved {
        Resolved(
            add: add ?? strings.contactMatchListAdd,
            message: message ?? strings.sendMessage,
            empty: empty ?? strings.contactMatchListEmpty
        )
    }
}
...
public struct AnnouncementReadBarView: View {
    ...
    private var copy: AnnouncementReadLabels.Resolved { labels.resolve(strings) }
    ...
    public var body: some View {
        ...
        HStack(spacing: FlareSizes.spacingSm) {
            ...
            if showCount {
                Text(copy.readCount(readCount, memberCount))
                    .font(.system(size: FlareSizes.fontSizeMd))
                    .foregroundColor(selfRead ? colors.textTertiary : colors.textSecondary)
            }
            Spacer()
...
public struct AnnouncementReadLabels: Sendable {
    ...
    public init(confirmRead: String? = nil,
                viewUnread: String? = nil,
                readCount: (@Sendable (Int, Int) -> String)? = nil) {
        self.confirmRead = confirmRead; self.viewUnread = viewUnread; self.readCount = readCount
    }
}

public extension AnnouncementReadLabels {
    /// Every label filled in: an explicit label wins, otherwise the `flareStrings` provider.
    struct Resolved: Sendable {
        public let confirmRead: String
        public let viewUnread: String
    ...
    func resolve(_ strings: FlareStrings) -> Resolved {
        Resolved(
            confirmRead: confirmRead ?? strings.announcementReadBarConfirmRead,
            viewUnread: viewUnread ?? strings.announcementReadBarViewUnread,
            readCount: readCount ?? strings.announcementReadBarReadCount
        )
    }
}
