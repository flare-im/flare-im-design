...
public struct TypingIndicatorView: View {
        ...
        HStack(spacing: 4) {
            ForEach(0..<3, id: \.self) { i in
                Circle()
                    ...
                    .animation(reduceMotion ? nil : .easeInOut(duration: 0.6).repeatForever().delay(Double(i) * 0.15), value: animating)
            }
        }
    ...
    public var body: some View {
        ...
        // Radius 16 with a 4pt bottom-leading tail, the same tail incoming bubbles use.
        .background(
            variant == .bubble
                ? AnyView(flareBubbleShape(.bottomLeading).fill(colors.bgPrimary)
                    .overlay(flareBubbleShape(.bottomLeading).stroke(colors.borderPrimary, lineWidth: 1)))
                : AnyView(Color.clear)
        )
