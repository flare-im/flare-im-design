    ...
    public var body: some View {
        ...
        VStack(spacing: 0) {
            HStack(spacing: FlareSizes.spacingSm) {
                ...
                TextField(searchPlaceholder ?? strings.startConversationSearchPlaceholder, text: $query)
            }
                    ...
                    HStack {
                        ...
                        if allowGroup {
                            Image(systemName: selected.contains(c.id) ? "checkmark.circle.fill" : "circle")
                                .foregroundColor(selected.contains(c.id) ? colors.primaryText : colors.textTertiary)
                        }
                    }
                }
                .buttonStyle(.plain)
