...
public struct StorageUsageView: View {
    ...
    public init(categories: [FlareStorageCategory],
                ...
                atLeastText: String? = nil,
                clearText: String? = nil,
                clearingText: String? = nil,
                totalText: String? = nil,
                deviceFreeText: String? = nil,
                reloadText: String? = nil,
                retryText: String? = nil,
                dismissErrorText: String? = nil,
                loadingText: String? = nil,
                emptyText: String? = nil,
                fileCountText: String? = nil,
    ...
    public func sizeText(_ bytes: Double?) -> String {
        formatBytes(bytes, locale: locale) ?? copy.unknownText
    }

    public func fileCountLabel(_ count: Int?) -> String? {
        guard let count, count >= 0 else { return nil }
        return copy.fileCountText.replacingOccurrences(of: "{count}", with: "\(count)")
    }
    ...
    public func totalLabel(_ totals: FlareStorageTotals) -> String {
        ...
        return totalIsFloor(totals) ? "\(copy.atLeastText) \(value)" : value
    }

    /// Copy in effect: an explicit parameter wins, otherwise the `flareStrings` provider.
    struct Copy {
        let title, unknownText, atLeastText, clearText: String
        let clearingText, totalText, deviceFreeText, reloadText: String
        let retryText, dismissErrorText, loadingText, emptyText: String
        let fileCountText: String
    }
    func resolveCopy(_ strings: FlareStrings) -> Copy {
        Copy(
            title: title ?? strings.storageUsageTitle,
            unknownText: unknownText ?? strings.storageUsageUnknownSize,
            atLeastText: atLeastText ?? strings.storageUsageAtLeast,
            clearText: clearText ?? strings.storageUsageClear,
            clearingText: clearingText ?? strings.storageUsageClearing,
            totalText: totalText ?? strings.storageUsageTotal,
            deviceFreeText: deviceFreeText ?? strings.storageUsageDeviceFree,
            reloadText: reloadText ?? strings.storageUsageReload,
            retryText: retryText ?? strings.storageUsageRetry,
            dismissErrorText: dismissErrorText ?? strings.storageUsageDismissError,
            loadingText: loadingText ?? strings.storageUsageLoading,
            emptyText: emptyText ?? strings.storageUsageEmpty,
            fileCountText: fileCountText ?? strings.storageUsageFileCount
        )
    }
    ...
    private func head(_ colors: FlareColors, _ totals: FlareStorageTotals) -> some View {
        HStack(alignment: .top, spacing: FlareSizes.spacingSm) {
            ...
            Spacer(minLength: 0)
            if let onReload {
                textButton(symbol: "arrow.clockwise", label: copy.reloadText,
    ...
    func headSummary(_ totals: FlareStorageTotals) -> String {
        let total = "\(copy.totalText) \(totalLabel(totals))"
        guard let free = formatBytes(deviceFreeBytes, locale: locale) else { return total }
        return "\(total) · \(copy.deviceFreeText) \(free)"
    }
    ...
    private func loadingView(_ colors: FlareColors) -> some View {
        ...
        .accessibilityLabel(copy.loadingText)
    }
        ...
        VStack(alignment: .leading, spacing: FlareSizes.spacingXs) {
            HStack(spacing: FlareSizes.spacingMd) {
                ...
                if category.busy {
                    HStack(spacing: 5) {
                        ProgressView().controlSize(.small)
                        Text(copy.clearingText)
                            .font(.system(size: FlareSizes.fontSizeSm))
                            .foregroundColor(colors.textTertiary)
                    }
                    .accessibilityElement(children: .combine)
                    .accessibilityLabel(copy.clearingText)
                } else if canClear {
    ...
    private func clearButton(_ category: FlareStorageCategory, colors: FlareColors) -> some View {
        ...
        .buttonStyle(.plain)
        .accessibilityLabel("\(copy.clearText) \(category.label)")
    }
    ...
    private func rowError(_ category: FlareStorageCategory, message: String,
        ...
        HStack(spacing: 6) {
            ...
            if canClear, !category.busy {
                textButton(symbol: "arrow.clockwise", label: copy.retryText,
