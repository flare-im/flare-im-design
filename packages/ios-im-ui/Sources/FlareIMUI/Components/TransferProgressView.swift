...
public enum FlareTransferState: String, CaseIterable, Sendable {
    case idle, queued, transferring, paused, failed, completed, cancelled
...
public struct TransferProgressView: View {
    ...
    public var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let value { ProgressView(value: value).tint(colors.primaryText).accessibilityLabel(name + " — " + statusText) }
            else if state == .transferring { ProgressView().tint(colors.primaryText).accessibilityLabel(name + " — " + statusText) }
