    ...
    private func rail(_ colors: FlareColors) -> some View {
        VStack(spacing: 6) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                // Each tab is a 44pt target, so the tabs abut; the 38pt grounds keep their gaps.
                HStack(spacing: 0) {
        ...
        return Button { activeKey = pack.key } label: {
            Group {
                if pack.key == Self.recentKey {
                    Image(systemName: flareIconSymbol("clock")).font(.system(size: 16)).foregroundColor(colors.textSecondary)
