...
public struct EmojiPickerView: View {
    ...
    private func rail(_ colors: FlareColors) -> some View {
        VStack(spacing: 6) {
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                // Each tab is a 44pt target, so the tabs abut; the 32pt grounds keep their gaps.
                HStack(spacing: 0) {
                    if !recents.isEmpty {
                        tab(colors, key: Self.recentKey, label: strings.recent, symbol: flareIconSymbol("clock"), glyph: nil)
                    }
                    ForEach(categories) { cat in
                        tab(colors, key: cat.key, label: cat.label, symbol: cat.symbol,
    ...
    /// A category tab named by its category: the symbol, else the first emoji, else the category's own
    /// label (never a stand-in glyph); the active tab is selected.
    private func tab(_ colors: FlareColors, key: String, label: String, symbol: String?, glyph: String?) -> some View {
        ...
        return Button { activeKey = key } label: {
            Group {
                if let symbol {
                    ...
                    Text(label).font(.system(size: FlareSizes.fontSizeXs)).foregroundColor(colors.textSecondary)
