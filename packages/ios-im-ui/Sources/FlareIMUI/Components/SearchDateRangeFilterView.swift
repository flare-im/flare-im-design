...
public struct SearchDateRangeFilterView: View {
    ...
    public init(value: FlareSearchTimeRange, options: [FlareSearchRangeOption] = [],
                ...
                disabled: Bool = false, title: String? = nil, customText: String? = nil,
                fromLabel: String? = nil, toLabel: String? = nil,
                clearText: String? = nil, unlimitedText: String? = nil,
                invalidText: String? = nil,
    ...
    private var summary: String {
        if unrestrictedRange(value) { return copy.unlimitedText }
        ...
        var parts: [String] = []
        if !dates.from.isEmpty { parts.append("\(copy.fromLabel) \(dates.from)") }
        if !dates.to.isEmpty { parts.append("\(copy.toLabel) \(dates.to)") }
        return parts.isEmpty ? copy.unlimitedText : parts.joined(separator: " · ")
    }
    ...
    struct Copy {
        let title, customText, fromLabel, toLabel: String
        let clearText, unlimitedText, invalidText: String
    }
    func resolveCopy(_ strings: FlareStrings) -> Copy {
        Copy(
            title: title ?? strings.timeRange,
            customText: customText ?? strings.searchDateRangeFilterCustom,
            fromLabel: fromLabel ?? strings.searchDateRangeFilterFrom,
            toLabel: toLabel ?? strings.searchDateRangeFilterTo,
            clearText: clearText ?? strings.clear,
            unlimitedText: unlimitedText ?? strings.searchDateRangeFilterUnlimited,
            invalidText: invalidText ?? strings.searchDateRangeFilterInvalid
        )
    }
    ...
    public var body: some View {
        ...
        VStack(alignment: .leading, spacing: FlareSizes.spacingMd) {
            Text(copy.title).font(.system(size: FlareSizes.fontSizeXl, weight: .semibold))
            ...
            LazyVGrid(columns: [GridItem(.adaptive(minimum: 96), spacing: FlareSizes.spacingSm)],
                ...
                if allowCustom {
                    chip(copy.customText, icon: "calendar", selected: showCustom,
                         disabled: disabled, colors: colors) { customOpen.toggle() }
                        .accessibilityHint(showCustom ? strings.collapse : strings.expand)
                }
            }

            if showCustom {
                VStack(alignment: .leading, spacing: FlareSizes.spacingMd) {
                    field(copy.fromLabel, value: draft.from, colors: colors) { pick(from: $0) }
                    field(copy.toLabel, value: draft.to, colors: colors) { pick(to: $0) }
                }
            ...
            if invalid {
                HStack(alignment: .top, spacing: FlareSizes.spacingXs) {
                    ...
                    Text(copy.invalidText).font(.system(size: bodySize)).foregroundColor(colors.warningText)
                }
            ...
            HStack(spacing: FlareSizes.spacingSm) {
                ...
                if let onClear, !unrestrictedRange(value) {
                    Button(action: onClear) {
                        Text(copy.clearText).font(.system(size: bodySize))
        ...
        VStack(alignment: .leading, spacing: FlareSizes.spacingXs) {
            ...
            HStack(spacing: FlareSizes.spacingXs) {
                ...
                if !fieldValue.isEmpty {
                    ...
                    .buttonStyle(.plain)
                    .disabled(disabled)
                    .accessibilityLabel("\(copy.clearText) \(label)")
                }
            }
        }
    }
