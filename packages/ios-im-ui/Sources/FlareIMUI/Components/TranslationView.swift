...
public struct TranslationView: View {
    ...
    private func content(_ colors: FlareColors) -> some View {
        if pending {
            HStack(spacing: 6) {
                Image(systemName: flareIconSymbol("translate")).font(.system(size: 14)).foregroundColor(colors.textTertiary)
            ...
            VStack(alignment: .leading, spacing: 6) {
                ...
                HStack(spacing: 6) {
                    Image(systemName: flareIconSymbol("translate")).font(.system(size: 11)).foregroundColor(colors.textTertiary)
