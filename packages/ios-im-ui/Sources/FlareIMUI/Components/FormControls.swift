...
/// Ref: packages/vue-im-ui `FlareTextarea.vue`.
...
/// Spec: Form/Stepper (`StepperView`). Ref: packages/vue-im-ui `FlareStepper.vue`.
public struct StepperView: View {
    ...
    public var body: some View {
        let colors = FlareColors.of(scheme, brand: flareBrandTheme)
        HStack(spacing: 0) {
            button(colors, icon: "remove", label: strings.decrease, enabled: canDec) { set(value - step) }
            Text(display)
                .font(.system(size: size.font))
                .monospacedDigit()
                .foregroundColor(colors.textPrimary)
                .frame(width: fieldWidth, height: size.height)
            button(colors, icon: "add", label: strings.increase, enabled: canInc) { set(value + step) }
        }
    ...
    /// A −/+ key: the kit icon `icon`, named `label`. The key keeps the stepper box's size; its 44pt
    /// target reaches past the box, away from the other key.
    @ViewBuilder
    private func button(_ colors: FlareColors, icon: String, label: String, enabled: Bool,
        ...
        .buttonStyle(.plain)
        .flareCompactLayout(width: size.square, height: size.height)
        .disabled(!enabled)
...
/// Spec: Form/Slider (`SliderView`). Ref: packages/vue-im-ui `FlareSlider.vue`.
...
/// Star rating row. Spec: Form/Rating (`RatingView`). Ref: packages/vue-im-ui `FlareRating.vue`.
/// Uses SF Symbols `star.fill` / `star`; gold = `colors.warning`.
///
/// While it can be changed, every star is a button with a 44pt target, named by its value, and the
/// star equal to the rating is selected. Read-only or disabled, the row is one element whose value is
/// the rating out of `count`.
public struct RatingView: View {
    ...
    public var body: some View {
        let colors = FlareColors.of(scheme, brand: flareBrandTheme)
        let stars = Swift.max(1, count)
        // Interactive stars are 44pt targets side by side; read-only stars keep the compact row.
        let row = HStack(spacing: interactive ? 0 : FlareSizes.spacingXs) {
            ForEach(1...stars, id: \.self) { n in
                if interactive {
                    Button { pick(n) } label: { star(n, colors).flareTouchTarget() }
                        .buttonStyle(.plain)
                        .accessibilityLabel(strings.ratingStar(n))
        ...
        if interactive {
            row.accessibilityElement(children: .contain).accessibilityLabel(strings.rating)
        } else {
            row.accessibilityElement(children: .ignore)
                .accessibilityLabel(strings.rating)
                .accessibilityValue("\(value)/\(stars)")
        }
    }

    private func star(_ n: Int, _ colors: FlareColors) -> some View {
        Image(systemName: n <= value ? "star.fill" : flareIconSymbol("star"))
            .font(.system(size: size))
            .foregroundColor(n <= value ? colors.warningText : colors.borderHover)
    }
}
...
/// Ref: packages/vue-im-ui `FlareTimePicker.vue`.
///
...
/// Ref: packages/vue-im-ui `FlareDatePicker.vue`.
///
