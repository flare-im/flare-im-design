    ...
    public var body: some View {
        ...
        HStack(alignment: .top, spacing: FlareSizes.spacingMd) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 0) {
                    Text(strings.groupAnnouncement).font(.system(size: 12, weight: .semibold)).foregroundColor(colors.primaryText)
            ...
            if dismissible {
                Button { onClose?() } label: {
                    Image(systemName: flareIconSymbol("close")).font(.system(size: 16)).foregroundColor(colors.textTertiary)
                        .flareTouchTarget()
                }
                .buttonStyle(.plain)
                .flareCompactLayout(width: FlareSizes.iconSizeSm, height: FlareSizes.iconSizeSm)
                .accessibilityLabel(strings.announcementBannerDismiss)
            }
        }
