    ...
    private func cancelButton(_ colors: FlareColors) -> some View {
        Button { onCancel?() } label: {
            ZStack {
                ...
                Image(systemName: flareIconSymbol("delete")).font(.system(size: 18))
        ...
        .accessibilityLabel(strings.cancelRecording)
    }
        ...
        if cancelling {
            ...
            Button { onSend?() } label: {
                ZStack {
                    ...
                    Image(systemName: flareIconSymbol("send")).font(.system(size: 18)).foregroundColor(.white)
                }
                .frame(width: 36, height: 36)
                .flareTouchTarget()
            }
            .buttonStyle(.plain)
            .flareCompactLayout(width: 36, height: 36)
            .accessibilityLabel(strings.sendVoice)
        }
    }
}
