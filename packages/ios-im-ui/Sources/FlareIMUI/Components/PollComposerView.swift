        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack {
                ...
                Button { onCancel?() } label: {
                    Image(systemName: flareIconSymbol("close")).font(.system(size: 13, weight: .semibold))
                        .foregroundColor(colors.textTertiary)
                        .flareTouchTarget()
                }
                .buttonStyle(.plain)
                .flareCompactLayout(width: 13, height: 13)
            ...
            Button { multiple.toggle() } label: {
                HStack(spacing: 8) {
                    Image(systemName: multiple ? "checkmark.square" : "square")
                        .font(.system(size: 15)).foregroundColor(multiple ? colors.primaryText : colors.textTertiary)
        ...
        HStack(spacing: 8) {
            ...
            if options.count > 2 {
                Button { options.remove(at: i) } label: {
                    Image(systemName: flareIconSymbol("close")).font(.system(size: 11, weight: .semibold))
                        .foregroundColor(colors.textTertiary)
                        // The glyph keeps its place at the trailing edge; the target takes 44pt of the
                        // row's width (never the text field's) and reaches past the row's height.
                        .flareTouchTarget(alignment: .trailing)
                }
                .buttonStyle(.plain)
                .flareCompactLayout(height: 11)
