...
struct FlareDatePill_Previews: PreviewProvider {
    static var previews: some View {
        VStack { DatePillView(label: "今天"); DatePillView(label: "昨天"); DatePillView(label: "7月15日", floating: true) }.padding()
    }
}
...
struct FlareMessageStatus_Previews: PreviewProvider {
    static var previews: some View {
        HStack(spacing: 16) {
            ...
            MessageStatusView(status: .retrying)
        }.padding()
    }
}

struct FlareMessageMeta_Previews: PreviewProvider {
    static var previews: some View {
        MessageMetaView(
            timestamp: "14:32",
            edited: true,
...
struct FlareSearchBar_Previews: PreviewProvider {
    static var previews: some View {
        VStack(spacing: 12) {
            SearchBarView(text: .constant(""), placeholder: "搜索联系人、群组、消息")
            SearchBarView(text: .constant(""), placeholder: "搜索", readOnly: true, onActivate: {})
        }.padding()
    }
}

struct FlareBottomSheet_Previews: PreviewProvider {
    static var previews: some View {
        BottomSheetView(title: "消息操作") {
            MessageActionSheetView(availability: .init(canReply: true, canCopy: true, canDelete: true),
                                   content: FlareTextContent("周五的评审挪到下午三点"))
        }
    }
}
...
struct FlareContactItem_Previews: PreviewProvider {
    static var previews: some View {
        VStack {
            ContactItemView(item: Contact(id: "u1", name: "Henry Ford", signature: "Keep it simple.", presence: .online), onSelect: {})
            ContactItemView(item: Contact(id: "u2", name: "Ivy Chen", signature: "设计即沟通", presence: .busy),
                            selectable: true, selected: true, onToggleSelect: {})
            ContactItemView(item: Contact(id: "u3", name: "Kai"), showPresence: false,
                            trailing: AnyView(ButtonView(label: "移出", variant: .secondary, size: .sm, action: {})))
        }.padding()
    }
}
...
struct FlareNewFriendRequests_Previews: PreviewProvider {
    static var previews: some View {
        NewFriendRequestsView(items: [FriendRequest(id: "r1", name: "Ivy Chen", message: "一起做设计～"),
                                      FriendRequest(id: "r2", name: "Kai", message: "加个好友", direction: .outgoing)],
                              onAccept: { _ in }, onReject: { _ in }, onWithdraw: { _ in })
    }
}
...
struct FlareTextMessage_Previews: PreviewProvider {
    static var previews: some View {
        VStack(alignment: .leading, spacing: 8) {
            MessageBubbleView(message: demoRow("text-out", "Me", "收到，我下午过一遍给你反馈 👍", self: true), currentUserId: "me")
            MessageBubbleView(message: demoRow("text-in", "Ivy Chen", "新版设计稿已经上传啦，帮忙看下～"), currentUserId: "me")
        }.padding()
    }
}
...
struct FlareMessageBubble_Previews: PreviewProvider {
    static var previews: some View {
        VStack(spacing: 4) {
            ...
            MessageBubbleView(message: FlareMessageData(id: "3", senderId: "me", senderName: "Me", content: FlareTextContent("收到，我下午过一遍给你反馈 👍"),
                                                        timeLabel: "14:30", status: .read,
                                                        replyTo: FlareReplyTarget(senderName: "Ivy Chen", summary: "新版设计稿已经上传啦，帮忙看下～", messageId: "2")),
                              currentUserId: "me", conversationKind: .group, onLocateMessage: { _ in })
        }.padding()
    }
}
...
struct FlareChatHeader_Previews: PreviewProvider {
    static var previews: some View {
        ConversationHeaderView(identity: ConversationIdentity(id: "u2", title: "Ivy Chen", subtitle: "在线", presence: .online,
                                                              action: ConversationHeaderAction(id: "details", label: "会话详情")),
                               actions: [ConversationHeaderAction(id: "mute", label: "免打扰", icon: "mute", pressed: true)],
                               showBack: true, onBack: {}, onAction: { _ in })
    }
}

struct FlareActionMenu_Previews: PreviewProvider {
    static var previews: some View {
        ActionMenuView(items: [
            FlareActionItem(id: "addFriend", label: "加好友", icon: "person-add", group: "new"),
            FlareActionItem(id: "createGroup", label: "发起群聊", icon: "people", group: "new"),
            FlareActionItem(id: "requests", label: "新的联系人", icon: "notification", group: "manage", badge: "3"),
            FlareActionItem(id: "mute", label: "消息免打扰", group: "manage", pressed: true),
            FlareActionItem(id: "export", label: "导出聊天记录", icon: "download", group: "manage",
                            enabled: false, disabledReason: "仅群主可导出"),
            FlareActionItem(id: "leave", label: "退出群聊", icon: "logout", group: "danger", danger: true),
        ], accessibilityLabel: "更多操作", onSelect: { _ in }) {
            Image(systemName: flareIconSymbol("add"))
        }
        .padding()
    }
}
...
private struct FilterTabsDemo: View {
    ...
    private let options: [FlareFilterTabOption] = [
        ...
        FlareFilterTabOption(value: "groups", label: "群聊", icon: "group"),
        FlareFilterTabOption(value: "mentions", label: "@我", badge: 2),
        FlareFilterTabOption(value: "bots", label: "机器人", icon: "mini-app"),
    ]
...
private struct CommandPaletteDemo: View {
    ...
    @State private var selectedId: String? = "new-message"

    private let groups = [
        FlareCommandPaletteGroup(id: "workspace", label: "工作区", commands: [
            FlareCommandPaletteCommand(id: "new-message", label: "发起新会话",
                                       description: "选择联系人或群组", shortcut: "⌘N"),
            FlareCommandPaletteCommand(id: "search", label: "搜索消息",
                                       description: "在当前工作区中查找", shortcut: "⌘F"),
            FlareCommandPaletteCommand(id: "settings", label: "打开设置", shortcut: "⌘,"),
        ]),
    ]
    ...
    var body: some View {
        CommandPaletteView(
            ...
            onSelectedIdChange: { selectedId = $0 }
        )
    }
}

struct FlareCommandPalette_Previews: PreviewProvider {
    static var previews: some View {
        CommandPaletteDemo()
        CommandPaletteDemo().preferredColorScheme(.dark)
    }
}
#endif
