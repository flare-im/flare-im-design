 ...
 * (O(visible)).
 *
 * Rows render in host order (the core already sorts pinned conversations first); a pinned row keeps
 * its pin mark but is never moved by the list.
 */
...
fun ConversationList(
    ...
    /** Null takes the kit's copy (`conversationRowDraft` / `conversationRowMention`), like [ConversationRow]. */
    ...
    val emptyText = emptyText ?: strings.noConversations
    if (items.isEmpty()) {
