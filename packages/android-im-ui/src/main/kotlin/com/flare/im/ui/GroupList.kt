...
fun GroupList(
    ...
    /** Replaces the whole empty state, so it can say what to do about there being no group. */
    empty: (@Composable () -> Unit)? = null,
) {
    val strings = flareStrings()
    val emptyText = emptyText ?: strings.noGroups
    val colors = flareColors()
    if (items.isEmpty()) {
        if (empty != null) empty() else EmptyState(title = emptyText)
        return
    }
    LazyColumn(Modifier.fillMaxWidth()) {
        items(items, key = { it.id }) { g ->
            Row(
                ...
                Column {
                    Text(g.name, color = colors.textPrimary, fontWeight = FontWeight.Medium, fontSize = FlareSizes.fontSizeLg.value.sp)
