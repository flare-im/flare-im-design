...
fun EmptyState(
     ...
     * (centred, wrapping, one gap); the host owns the controls. [actionText] stays the one-button
     * shorthand, and both may be given.
     */
    ...
    Column(
        ...
        when {
            ...
            else -> FlareIcon(name = icon, size = 44.dp, tint = defaultIconTint)
        }
        Spacer(Modifier.height(FlareSizes.spacingSm))
        Text(title, color = if (isError) colors.errorText else colors.textPrimary, fontSize = FlareSizes.fontSize2xl.value.sp, textAlign = TextAlign.Center)
        if (description != null) {
            Spacer(Modifier.height(FlareSizes.spacingSm))
        ...
        if (actions != null) {
            Spacer(Modifier.height(FlareSizes.spacingLg))
            FlowRow(
                horizontalArrangement = Arrangement.spacedBy(FlareSizes.spacingSm, Alignment.CenterHorizontally),
