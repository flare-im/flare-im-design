...
export function getContentDecodedPreview(decoded: ContentElem | null | undefined, locale?: string): string {
  ...
  switch (type) {
    ...
    case "image_group": {
      // An album says how many it carries; one with no images says only what it is, rather than
      // "[多图] 0 张" (the rule the three native kits implement, and the shared vector table).
      const images = readArray(pickNestedPayload(decoded, "image_group"), "images").length || readArray(root, "images").length;
      return images > 0
        ? resolveFlareMessage(locale, "preview.imageGroupCount", { count: images })
        : resolveFlareMessage(locale, "preview.imageGroup");
    }
    case "system":
...
/** Plain-text extraction for copy/reply preview with no host runtime dependency. */
export function getMessageText(message: MessageTextSource): string {
  ...
  // Nothing readable: an id is not text, so copy and previews get nothing rather than it.
  return "";
}
