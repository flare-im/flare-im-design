...
import { TrashOutline, SendOutline, MicOutline } from "../../shared/icon-glyphs";
...
<template>
  <div class="flare-voice-rec" :class="{ 'is-cancelling': cancelling }" role="group">
    <button type="button" class="flare-voice-rec__cancel" :aria-label="t('recording.cancel')" @click="emit('cancel')">
      <n-icon aria-hidden="true" :size="18" :component="TrashOutline" />
    </button>
    ...
    <button
      ...
      @click="emit('send')"
    >
      <n-icon aria-hidden="true" :size="18" :component="SendOutline" />
    </button>
  </div>
</template>
...
.flare-voice-rec.is-cancelling {
  ...
  border-color: color-mix(in srgb, var(--flare-color-error) 40%, transparent);
}
...
.flare-voice-rec.is-cancelling .flare-voice-rec__wave i { background: var(--flare-color-error); }
