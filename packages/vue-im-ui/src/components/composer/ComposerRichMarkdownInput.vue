...
function parseToken(token: string): RichSegment | null {
  ...
  if (linkedImageMatch) {
    return {
      kind: "image",
      token,
      label: linkedImageMatch[1]?.trim() || t("composer.formatImage"),
  ...
  if (imageMatch) {
    return {
      kind: "image",
      token,
      label: imageMatch[1]?.trim() || t("composer.formatImage"),
  ...
  if (linkMatch) {
    return {
      kind: "link",
      token,
      label: linkMatch[1]?.trim() || t("composer.formatLink"),
...
function parseRichTextLine(source: string): RichSegment[] {
  ...
  if (props.formattingPreview) {
    ...
    if (dividerMatch) {
      out.push({ kind: "divider", token: source, label: t("composer.formatDivider") });
      return out;
    }

    const orderedMatch = /^\s{0,3}(\d+)\.\s+([\s\S]+)$/.exec(source);
    if (orderedMatch) {
      out.push({ kind: "ordered", token: source, label: orderedMatch[2] || t("composer.sample.listItem"), meta: `${orderedMatch[1]}.` });
      return out;
    }

    const bulletMatch = /^\s{0,3}[-*+]\s+([\s\S]+)$/.exec(source);
    if (bulletMatch) {
      out.push({ kind: "bullet", token: source, label: bulletMatch[1] || t("composer.sample.listItem"), meta: "•" });
      return out;
    }

    const quoteMatch = /^\s{0,3}>\s?([\s\S]+)$/.exec(source);
    if (quoteMatch) {
      out.push({ kind: "quote", token: source, label: quoteMatch[1] || t("composer.sample.quote") });
      return out;
    }
  }
...
function applyFormat(key: RichMarkdownFormatKey): void {
  ...
  else if (key === "link") replaceRangeWithMarkdown(range, `[${hasSelection ? selectedText : t("composer.sample.link")}](https://)`, !hasSelection);
  else if (key === "image") replaceRangeWithMarkdown(range, `![${hasSelection ? selectedText : t("composer.sample.image")}](https://)`, !hasSelection);
  else if (key === "ordered") replaceRangeWithMarkdown(range, linesWithPrefix(selectedText, (index) => `${index + 1}. `, t("composer.sample.listItem")), !hasSelection);
  else if (key === "bullet") replaceRangeWithMarkdown(range, linesWithPrefix(selectedText, () => "- ", t("composer.sample.listItem")), !hasSelection);
  else if (key === "quote") replaceRangeWithMarkdown(range, linesWithPrefix(selectedText, () => "> ", t("composer.sample.quote")), !hasSelection);
  else if (key === "codeBlock") replaceRangeWithMarkdown(range, `\n\`\`\`\n${hasSelection ? selectedText : t("composer.sample.code")}\n\`\`\`\n`, !hasSelection);
