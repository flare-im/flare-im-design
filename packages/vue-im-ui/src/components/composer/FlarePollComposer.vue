...
<template>
  <div class="flare-poll-composer">
    ...
    <button v-if="canAdd" type="button" class="flare-poll-composer__add" @click="addOption">
      <n-icon aria-hidden="true" :size="16" :component="AddOutline" />{{ t("poll.addOption") }}
    </button>

    <label class="flare-poll-composer__multi">
      <input v-model="multiple" type="checkbox" />
      <n-icon aria-hidden="true" :size="16" :component="CheckboxOutline" />
...
.flare-poll-composer__question {
  border: none;
  border-bottom: 1.5px solid var(--flare-color-border-primary);
  padding: 6px 2px;
  font-size: 15px;
  font-weight: 500;
  color: var(--flare-color-text-primary);
...
.flare-poll-composer__question:focus { border-color: var(--flare-color-primary); }
...
.flare-poll-composer__option:focus-within { box-shadow: 0 0 0 2px var(--flare-color-focus-ring); }
