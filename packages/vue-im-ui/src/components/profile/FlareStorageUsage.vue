...
const strings = computed(() => ({
  title: props.title ?? t("storageUsage.title"),
  unknownText: props.unknownText ?? t("storageUsage.unknown"),
  atLeastText: props.atLeastText ?? t("storageUsage.atLeast"),
  clearText: props.clearText ?? t("storageUsage.clear"),
  clearingText: props.clearingText ?? t("storageUsage.clearing"),
  totalText: props.totalText ?? t("storageUsage.total"),
  deviceFreeText: props.deviceFreeText ?? t("storageUsage.deviceFree"),
  reloadText: props.reloadText ?? t("storageUsage.reload"),
  retryText: props.retryText ?? t("storageUsage.retry"),
  dismissErrorText: props.dismissErrorText ?? t("storageUsage.dismissError"),
  loadingText: props.loadingText ?? t("storageUsage.loading"),
  emptyText: props.emptyText ?? t("storageUsage.empty"),
  fileCountText: props.fileCountText ?? t("storageUsage.fileCount"),
}));
...
function sizeText(bytes: number | null | undefined): string {
  return formatBytes(bytes, props.locale) ?? strings.value.unknownText;
}
const totalValueText = computed(() => {
  const text = sizeText(totals.value.total);
  return totalIsFloor.value ? `${strings.value.atLeastText} ${text}` : text;
});
...
function fileCountLabel(count: number | undefined): string | null {
  ...
  return strings.value.fileCountText.replace("{count}", String(Math.round(count)));
}
...
<template>
  <section class="flare-storage" :aria-label="strings.title">
    <header class="flare-storage__head">
      <div class="flare-storage__headline">
        <h3 class="flare-storage__title">{{ strings.title }}</h3>
        <p class="flare-storage__totals">
          <span class="flare-storage__total">{{ strings.totalText }} {{ totalValueText }}</span>
          <span v-if="deviceFreeLabel" class="flare-storage__free">
            · {{ strings.deviceFreeText }} {{ deviceFreeLabel }}
          </span>
        </p>
      </div>
      <button
    ...
    <p v-else-if="showEmpty" class="flare-storage__empty" role="status">{{ strings.emptyText }}</p>
      ...
      >
        <div class="flare-storage__main">
          <span class="flare-storage__icon" aria-hidden="true">
            <n-icon aria-hidden="true" :size="18" :component="DocumentOutline" />
          </span>
...
.flare-storage__empty {
  margin: 0;
  padding: var(--flare-size-spacing-md) 0;
...
@media (pointer: coarse) {
  .flare-storage__clear { min-height: var(--flare-size-layout-touch-target); }
}
...
.flare-storage__error-icon { color: var(--flare-color-error-text); flex: none; }
