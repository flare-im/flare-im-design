...
<template>
  <div ref="root" class="flare-tp" :class="[`flare-tp--${rsize}`, { 'is-open': open, 'is-disabled': disabled }]">
    <button type="button" class="flare-tp__trigger" :disabled="disabled" @click.stop="show">
      <n-icon aria-hidden="true" :size="16" :component="TimeOutline" class="flare-tp__clock" />
    ...
    <transition v-if="!asSheet" name="flare-tp-pop">
      <div v-if="open" class="flare-tp__pop" @click.stop>
        <div class="flare-tp__cols">
          <div ref="hourCol" class="flare-tp__col" role="listbox" :aria-label="t('timePicker.hour')">
          ...
          <div ref="minCol" class="flare-tp__col" role="listbox" :aria-label="t('timePicker.minute')">
        ...
        <div class="flare-tp__footer">
          <button type="button" class="flare-tp__btn is-ghost" @click="close">{{ t("timePicker.cancel") }}</button>
          <button type="button" class="flare-tp__btn is-primary" @click="confirm">{{ t("timePicker.done") }}</button>
        </div>
      </div>
    </transition>
...
.flare-tp--sm .flare-tp__trigger { height: 32px; padding: 0 var(--flare-size-spacing-2sm); font-size: 13px; }
.flare-tp--md .flare-tp__trigger { height: 40px; padding: 0 12px; font-size: 14px; }
.flare-tp--lg .flare-tp__trigger { height: 48px; padding: 0 var(--flare-size-spacing-2md); font-size: 15px; }
...
.flare-tp__clock { color: var(--flare-color-text-tertiary); flex: 0 0 auto; }
.flare-tp__value { font-variant-numeric: tabular-nums; }
.flare-tp__value.is-placeholder { color: var(--flare-color-text-tertiary); }
...
.flare-tp__sep { align-self: center; color: var(--flare-color-text-tertiary); font-weight: 600; }
.flare-tp__cell {
  flex: 0 0 auto;
  border: none;
...
.flare-tp__cell:hover { background: var(--flare-color-bg-secondary); color: var(--flare-color-text-primary); }
...
.flare-tp__footer { display: flex; gap: 8px; padding: 8px; border-top: 1px solid var(--flare-color-border-primary); }
.flare-tp__footer.is-sheet { padding: var(--flare-size-spacing-2sm) 12px calc(12px + env(safe-area-inset-bottom, 0px)); }
.flare-tp__btn {
  flex: 1;
  height: 36px;
  border-radius: var(--flare-size-radius-lg);
