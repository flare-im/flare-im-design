...
<template>
  ...
  >
    <button
      ...
      @click="pick(n)"
    >
      <n-icon aria-hidden="true" :component="n <= active ? Star : StarOutline" />
    </button>
  </div>
</template>
...
.flare-rating__star {
  ...
  line-height: 1;
  color: var(--flare-color-border-hover);
  cursor: pointer;
  transition: color var(--flare-transition-fast), transform var(--flare-transition-fast);
}
.flare-rating__star.is-on { color: var(--flare-color-warning-text); }
