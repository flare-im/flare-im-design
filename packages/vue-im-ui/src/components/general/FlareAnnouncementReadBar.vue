...
const props = defineProps<{
  ...
  memberCount: number;
  selfRead: boolean;
  /** 是否允许「查看未读」，一般仅管理员；宿主还要处理 viewUnread，入口才出现。 */
...
// A view-unread button nobody handles would do nothing, so it needs the permission and a listener.
const offersViewUnread = computed(() => props.canViewUnread === true && Boolean(instance?.vnode.props?.onViewUnread));
...
<template>
  <div class="flare-announcement-read-bar" :class="{ 'is-read': selfRead }">
    <NIcon aria-hidden="true" class="flare-announcement-read-bar__icon" :size="16">
