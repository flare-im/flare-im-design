...
import FlareSystemMessage from "../../standalone/FlareSystemMessage.vue";

// Timeline adapter: resolves the system text from the content elem / stored preview and
// renders the contract body (FlareSystemMessage) — the same pill the native kits draw.
...
const body = computed(() => {
  ...
  // Host social system events (friendship_established / group.created / member_joined…) carry their
...
<template>
  <div class="im-system"><FlareSystemMessage :text="body" /></div>
</template>

<style scoped>
.im-system { text-align: center; }
</style>
