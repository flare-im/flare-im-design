...
const props = withDefaults(
  defineProps<{ emoji?: string; loadSrc?: AssetUrlLoader; fallbackText?: string; alt?: string; animated?: boolean }>(),
  { emoji: "🎉", fallbackText: "", alt: "", animated: true },
);
...
let generation = 0;
watch(() => props.loadSrc, async (loader) => {
  const current = ++generation;
  resolvedSrc.value = "";
  const url = (await loader?.())?.trim() ?? "";
  if (current === generation) resolvedSrc.value = url;
}, { immediate: true });
onBeforeUnmount(() => { generation += 1; });
</script>
<template>
  <component :is="interactive ? 'button' : 'div'" class="fm-emoji" :type="interactive ? 'button' : undefined" @click="interactive && emit('click')">
    <img v-if="loadSrc && animated && resolvedSrc" class="fm-emoji__thumb" :src="resolvedSrc" :alt="alt" loading="lazy" decoding="async" />
...
.fm-emoji__thumb { width: min(5.5em, 120px); height: min(5.5em, 120px); font-size: var(--flare-size-font-size-lg); object-fit: contain; }
