  ...
  <section class="business-detail-block" :class="`business-detail-block--status-${statusTone}`">
    ...
    >
      ...
      >
        ...
        <n-icon aria-hidden="true" :component="expanded ? ChevronUpOutline : ChevronDownOutline" />
      </button>
      <button
        v-if="actionLabel"
      ...
      >
        ...
        <n-icon aria-hidden="true" :component="ChevronForwardOutline" />
      </button>
    </footer>
  </section>
</template>
...
.business-detail-block__status {
  ...
  font-weight: 600;
  color: var(--flare-color-text-secondary);
  background: color-mix(in srgb, var(--flare-color-bg-primary) 76%, transparent);
}
...
.business-detail-block__rows {
  ...
  margin: 0;
  overflow: hidden;
  border: 1px solid color-mix(in srgb, var(--flare-color-border-primary) 54%, transparent);
  border-radius: 8px;
  background: color-mix(in srgb, var(--flare-color-bg-tertiary) 44%, var(--flare-color-bg-primary));
}
...
.business-detail-block__action {
  ...
  line-height: 1;
  transition:
    color var(--flare-component-motion-fast),
    background var(--flare-component-motion-fast);
}

.business-detail-block__toggle {
  flex: 0 0 auto;
  border-right: 1px solid color-mix(in srgb, var(--flare-color-border-primary) 46%, transparent);
}

.business-detail-block__action {
  flex: 1 1 auto;
  justify-content: flex-end;
  color: color-mix(in srgb, var(--flare-color-primary-text) 88%, var(--flare-color-text-primary));
}
...
.business-detail-block__action:focus-visible {
  background: color-mix(in srgb, var(--flare-color-primary) 6%, transparent);
  outline: none;
}
