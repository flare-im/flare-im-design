...
const strings = computed(() => ({
  unknownText: props.unknownText ?? t("unknownUserPlaceholder.unknown"),
  deactivatedText: props.deactivatedText ?? t("unknownUserPlaceholder.deactivated"),
  blockedText: props.blockedText ?? t("unknownUserPlaceholder.blocked"),
  unreachableText: props.unreachableText ?? t("unknownUserPlaceholder.unreachable"),
}));
...
const title = computed(() => {
  switch (presentation.value.kind) {
    case "deactivated": return strings.value.deactivatedText;
    case "blocked": return strings.value.blockedText;
    case "unreachable": return strings.value.unreachableText;
    default: return strings.value.unknownText;
  }
});
...
<template>
  ...
  >
    <span class="flare-unknown-user__avatar" aria-hidden="true">
      <n-icon aria-hidden="true" :size="density === 'card' ? 28 : 20" :component="PersonOutline as any" />
    </span>
    <div class="flare-unknown-user__body">
      <p class="flare-unknown-user__title">
        <span class="flare-unknown-user__badge" aria-hidden="true">
          <n-icon aria-hidden="true" :size="14" :component="glyphs[presentation.icon] as any" />
        </span>
...
.flare-unknown-user--card {
  ...
  padding: var(--flare-size-spacing-xl) var(--flare-size-spacing-lg);
...
.flare-unknown-user__avatar {
  ...
  place-items: center;
  flex: none;
  width: var(--flare-size-layout-avatar-size);
  height: var(--flare-size-layout-avatar-size);
