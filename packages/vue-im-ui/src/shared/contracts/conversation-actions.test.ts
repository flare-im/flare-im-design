...
describe("conversationActions", () => {
  ...
  it("offers markUnread only when nothing is unread", () => {
    expect(ids(base, { markRead: true, markUnread: true })).toEqual(["markUnread"]);
    expect(ids({ ...base, unreadCount: 3 }, { markRead: true, markUnread: true })).toEqual(["markRead"]);
    expect(ids({ ...base, unreadCount: 3 }, { markUnread: true })).toEqual([]);
  });

  it("keeps a stable order with clearHistory and delete last and flagged danger", () => {
    const entries = conversationActions({ ...base, unreadCount: 2 }, { ...all, clearHistory: true });
    expect(entries.map((e) => e.action)).toEqual(["pin", "mute", "markRead", "archive", "hide", "clearHistory", "delete"]);
    expect(entries.filter((e) => e.danger).map((e) => e.action)).toEqual(["clearHistory", "delete"]);
