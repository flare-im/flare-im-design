/** Presentation states supplied by the host adapter; the UI never starts a transfer. */
export type TransferState = 'idle' | 'queued' | 'transferring' | 'paused' | 'failed' | 'completed' | 'cancelled';
...
export const transferActions: Record<TransferState, readonly TransferAction[]> = {
  idle: [], queued: ['cancel'], transferring: ['pause', 'cancel'], paused: ['resume', 'cancel'],
