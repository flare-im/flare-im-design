...
export interface WorkspacePaneState {
  ...
  description?: string;
  /**
   * Label of the one action this state offers: retry on a failure, the next step
   * on an empty pane. Without it no button is offered, only the reason.
   */
  actionLabel?: string;
}
 ...
 */
export function paneEmptyActionVisible(state: WorkspacePaneState | null | undefined, hasAction: boolean): boolean {
  if (!hasAction) return false;
  if (paneRender(state) !== "empty") return false;
  const label = state?.actionLabel;
  return typeof label === "string" && label.trim().length > 0;
}
