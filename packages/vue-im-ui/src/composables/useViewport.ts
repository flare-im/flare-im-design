import {
  computed,
  inject,
  onMounted,
  onUnmounted,
  provide,
  readonly,
  ref,
  type InjectionKey,
  type Ref,
} from "vue";

import type { FlareApplicationResponsiveMode } from "../shared/contracts/application";
import {
  FLARE_BREAKPOINT_DESKTOP_MIN,
  FLARE_BREAKPOINT_TABLET_MIN,
} from "../shared/contracts/layout";

/**
 * The same thresholds `useFlareAdaptive` resolves h5 / ipad / pc from — one
 * source in shared/contracts/layout.ts, so a width cannot be "desktop" to the
 * stylesheet and "iPad" to the components at the same time.
 */
export const BREAKPOINT_DESKTOP_PX = FLARE_BREAKPOINT_DESKTOP_MIN;
export const BREAKPOINT_TABLET_PX = FLARE_BREAKPOINT_TABLET_MIN;

export type ViewportMode = "mobile" | "tablet" | "desktop";

export type ViewportContext = {
  width: Readonly<Ref<number>>;
  mode: Readonly<Ref<ViewportMode>>;
  isDesktop: Readonly<Ref<boolean>>;
  isMobile: Readonly<Ref<boolean>>;
  isTablet: Readonly<Ref<boolean>>;
};

const viewportKey: InjectionKey<ViewportContext> = Symbol("flare-viewport");

function readWidth(): number {
  if (typeof window === "undefined") return BREAKPOINT_DESKTOP_PX;
  // `innerWidth` can read 0 before the window/iframe has laid out; a 0 would
  // wrongly classify as mobile and stick until a resize. Fall back to the layout
  // width, then to the desktop breakpoint, so first paint isn't misclassified.
  return window.innerWidth || document.documentElement?.clientWidth || BREAKPOINT_DESKTOP_PX;
}

function resolveMode(width: number): ViewportMode {
  if (width >= BREAKPOINT_DESKTOP_PX) return "desktop";
  if (width >= BREAKPOINT_TABLET_PX) return "tablet";
  return "mobile";
}

function applyDocumentViewport(mode: ViewportMode, width: number): void {
  if (typeof document === "undefined") return;
  const root = document.documentElement;
  root.dataset.viewport = mode;
  root.style.setProperty("--viewport-width", `${width}px`);
}

export function useViewportProvider(): ViewportContext {
  const width = ref(readWidth());
  const mode = computed(() => resolveMode(width.value));
  const isDesktop = computed(() => mode.value === "desktop");
  const isMobile = computed(() => mode.value === "mobile");
  const isTablet = computed(() => mode.value === "tablet");

  function sync(): void {
    width.value = readWidth();
    applyDocumentViewport(mode.value, width.value);
  }

  onMounted(() => {
    sync();
    window.addEventListener("resize", sync, { passive: true });
    window.addEventListener("orientationchange", sync);
  });

  onUnmounted(() => {
    window.removeEventListener("resize", sync);
    window.removeEventListener("orientationchange", sync);
  });

  applyDocumentViewport(mode.value, width.value);

  const ctx: ViewportContext = {
    width: readonly(width),
    mode: readonly(mode),
    isDesktop: readonly(isDesktop),
    isMobile: readonly(isMobile),
    isTablet: readonly(isTablet),
  };
  provide(viewportKey, ctx);
  return ctx;
}

/** The viewport mode a shell's measured mode stands for. */
function flareViewportModeForShellMode(mode: FlareApplicationResponsiveMode): ViewportMode {
  if (mode === "mobile") return "mobile";
  if (mode === "tablet") return "tablet";
  return "desktop";
}

/**
 * Called by an application shell (FR-139): inside it the viewport mode is the shell's measured mode, and — until the
 * shell has measured — the mode of the context above (the window). `width` stays the window's. The document's
 * `data-viewport` attribute is the provider's and is left alone.
 *
 * Internal: provided through `provideFlareShell`, not exported from the package.
 */
export function provideFlareShellViewport(
  responsiveMode: Readonly<Ref<FlareApplicationResponsiveMode | undefined>>,
): ViewportContext {
  const outer = inject(viewportKey, null);
  const windowWidth = outer?.width ?? readonly(ref(readWidth()));
  const mode = computed<ViewportMode>(() => {
    const shell = responsiveMode.value;
    if (shell !== undefined) return flareViewportModeForShellMode(shell);
    return outer?.mode.value ?? resolveMode(windowWidth.value);
  });
  const ctx: ViewportContext = {
    width: windowWidth,
    mode: readonly(mode),
    isDesktop: readonly(computed(() => mode.value === "desktop")),
    isMobile: readonly(computed(() => mode.value === "mobile")),
    isTablet: readonly(computed(() => mode.value === "tablet")),
  };
  provide(viewportKey, ctx);
  return ctx;
}

export function useViewport(): ViewportContext {
  const ctx = inject(viewportKey);
  if (!ctx) {
    throw new Error("useViewport() requires useViewportProvider() at the app root");
  }
  return ctx;
}
