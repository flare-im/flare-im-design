...
// @lucide/vue. It exits non-zero on any gap so a missing icon can't slip in.
...
// When adding: verify the Lucide name against node_modules/@lucide/vue/dist/
...
const MAP = {
  ...
  FolderOutline: ["Folder"], GiftOutline: ["Gift"], HappyOutline: ["FaceSlightlySmiling"], Heart: ["Heart", true],
  ...
  ThumbsUpOutline: ["ThumbsUp"], TimeOutline: ["Clock"], TrashOutline: ["Trash"], VideocamOffOutline: ["VideoOff"],
  ...
  PodiumOutline: ["Vote"],
  // Semantic registry additions (Round 5): glyphs the kit draws for concepts that had no name.
  AddReactionOutline: ["FaceSlightlySmilingPlus"], AlertOctagonOutline: ["OctagonAlert"], ArchiveRestoreOutline: ["ArchiveRestore"],
  BarChartOutline: ["ChartColumn"], CallEndOutline: ["PhoneOff"], ChatbubbleDotOutline: ["MessageSquareDot"],
  ChatbubbleOffOutline: ["MessageSquareOff"], ChecklistOutline: ["ListChecks"], EraserOutline: ["Eraser"],
  GitMergeOutline: ["Merge"], GlobeOutline: ["Globe"], KeyOutline: ["KeyRound"], KeyboardOutline: ["Keyboard"],
  PersonRemoveOutline: ["UserMinus"], PinOffOutline: ["PinOff"], RotateOutline: ["RotateCw"],
  ScreenShareOutline: ["ScreenShare"], UndoOutline: ["Undo2"], ZoomInOutline: ["ZoomIn"], ZoomOutOutline: ["ZoomOut"],
  // Semantic registry additions (Round 9): concepts that were drawn under a name meaning something else.
  BugOutline: ["Bug"], HashOutline: ["Hash"], IdCardOutline: ["Contact"], ServerOutline: ["HardDrive"],
};
...
const lucideRoot = path.dirname(require.resolve("@lucide/vue/package.json"));
const dts = path.join(lucideRoot, "dist/lucide-vue.suffixed.d.ts");
if (existsSync(dts)) {
  ...
  if (bad.length) { console.error("Lucide target(s) not found in installed @lucide/vue:", bad.join(", ")); process.exit(1); }
} else {
  console.warn("@lucide/vue not installed; skipping Lucide-name validation.");
}
...
import {
${lucideNamedImports}} from "@lucide/vue";
...
if (process.argv.includes("--check")) {
  if (!existsSync(OUT) || readFileSync(OUT, "utf8") !== generated) {
    console.error(`${path.relative(ROOT, OUT)} is stale; run npm run gen:icons.`);
    process.exit(1);
  }
  console.log(`${path.relative(ROOT, OUT)} is current: ${lines.length} icons, ${used.size} used in src.`);
} else {
