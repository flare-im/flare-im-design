...
const conversation: FlareConversationDetailsModel = {
  conversationId: "c1", channelId: "g1", displayName: "Design", conversationType: "GROUP", membersCount: 12,
};
...
describe("FlareConversationDetails", () => {
  ...
  it("still honours the deprecated connectionTone (default → neutral) unless tone is set", () => {
    expect(mountDetails({ connectionTone: "default" }).findComponent(NTag).attributes("data-tone")).toBe("neutral");
    host.unmount();
    expect(mountDetails({ connectionTone: "warning" }).findComponent(NTag).attributes("data-tone")).toBe("warning");
    host.unmount();
    expect(mountDetails({ connectionTone: "success", tone: "info" }).findComponent(NTag).attributes("data-tone")).toBe("info");
  });
