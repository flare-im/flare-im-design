...
describe("FlareAvatar", () => {
  ...
  it("maps the deprecated showStatus + status pair onto presence", () => {
    wrapper = mount(FlareAvatar, { props: { userId: "u1", showStatus: true, status: "busy" } });
    expect(wrapper.find(".im-avatar__status").attributes("data-presence")).toBe("busy");
    wrapper.unmount();
    wrapper = mount(FlareAvatar, { props: { userId: "u1", showStatus: false, status: "busy" } });
    expect(wrapper.find(".im-avatar__status").exists()).toBe(false);
    wrapper.unmount();
    wrapper = mount(FlareAvatar, { props: { userId: "u1", showStatus: true, status: "busy", presence: "online" } });
    expect(wrapper.find(".im-avatar__status").attributes("data-presence")).toBe("online");
  });
